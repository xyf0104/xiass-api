<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.2.6-0ea5e9" alt="当前版本 v1.2.6" />
...
- 管理员批量添加已有 OpenAI OAuth 账号，最多三个独立授权窗口并行；保留邮箱，加密保存密码和 2FA 密钥，兼容既有 401 重新授权。
...
3. 点击“开始授权”。每个账号使用独立 OAuth state 和独立隐私上下文，最多三个并行。授权时的代理与账号保存的代理一致；带认证的 SOCKS5 使用任务期间的本机转接，不会回退直连。
4. 进入手机号节点后自动使用独立接码会话。号码被拒绝时自动换号；短信等待满 180 秒后自动取消旧号码，并以新的 OAuth state 和隐私上下文从头重试。
5. CAPTCHA、邮件验证码、账号限制或异常页面会先关闭临时上下文并自动重试一次；再次出现时保留明确错误供管理员核对。已经落库的账号不会被自动删除或重复创建。
4. 手机验证的领取、换号和取消均经过 XIASS 站内确认。号码和验证码按任务隔离；短信等待三分钟超时后关闭当前授权窗口，确认取消接码后可重试。原 Team 工作流的自动接码规则保持不变。
5. 授权成功后读取并核验实际账号配置；保留邮箱，密码和 2FA 密钥通过专用加密字段保存。401 时可复用原“一键重新授权”，更新原账号，不另建账号。密码和 2FA 密钥不进入普通账号列表、审计正文或导出文件。
