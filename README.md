<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.0.86-0ea5e9" alt="当前版本 v1.0.86" />
...
v1.0.86 完整保留 v1.0.85 的全部 XIASS API 二开能力，修复 `gpt-image-2` 分辨率档位计费：顾客明确请求 `1K/2K/4K` 时严格按请求档位计费；传入具体尺寸时按请求尺寸归档，其中 `1280x720`、`1536x1024` 等归入 1K；仅在请求为 `auto` 或未指定时读取实际输出图片尺寸，仍无法识别则按 OpenAI 默认 1K。前端、FRP、数据库结构和历史账单均未改动，XIASS Codex 配置助手继续独立维护在 v1.0.91。
...
4. 自动扫描普通会话和归档会话，并兼容旧版 `~/.codex/state_5.sqlite` 与新版 `~/.codex/sqlite/` 会话数据库。
...
9. 助手页面提供“立即修复历史会话并重启 Codex”，也可以选择任意历史配置备份恢复；这两个入口都会先执行相同的会话保护流程。

助手还提供独立的手动配置入口，可填写任意兼容 Responses API 的 Base URL、API 密钥和模型。公网地址必须使用 HTTPS；本机 `localhost`、`127.0.0.0/8` 和 `::1` 映射可使用 HTTP，并保留用户填写的 `/v1`、`/V1` 等路径。手动填写的密钥只提交到随机的 `127.0.0.1` 本机端口，不会发送给 XIASS API。网站选择的密钥通过回环地址的 URL Fragment 交给助手，不会进入 Nginx 或浏览器请求日志。
