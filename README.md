<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.0.70-0ea5e9" alt="当前版本 v1.0.70" />
...
v1.0.70 统一控制台下拉框、筛选层和弹窗的半透明显示，修复底层文字穿透，并优化浅色/深色动态背景的循环过渡。升级不修改后端业务逻辑、数据库结构和持久化目录。
...
> 为保证 v1.0.67 及更早版本可直接原地更新，运行目录、Compose service、容器名和部分 `NOWIND_*` 环境变量继续保留历史兼容标识。它们只影响内部升级契约，不会改变产品名称，也不会连接任何外部 XIASS 实例。
...
docker compose -f docker-compose.local.yml logs --tail 200 nowind-api
ss -lntp | grep ':8080'
```
...
该脚本会先停止容器并创建一致性完整备份，再同步部署文件、拉取镜像、重建应用并执行健康检查。旧版本升级时会继续识别历史运行时名称；失败时会尽量恢复原 Git 状态与旧栈。数据不会被删除。
...
```bash
cd /opt/nowind-api/deploy
docker compose -f docker-compose.local.yml pull nowind-api
docker compose -f docker-compose.local.yml up -d --no-deps --force-recreate nowind-api
```
