<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.2.12-0ea5e9" alt="当前版本 v1.2.12" />
...
- 管理员批量添加已有 OpenAI OAuth 账号，最多三个独立授权窗口并行；逐账号显示实时阶段、耗时与安全错误原因，保留邮箱并加密保存密码和 2FA 密钥，兼容既有 401 重新授权。
- 自定义号池、混合 Pro/Plus/Team 成员和号池级批量代理设置，不改变业务分组与客户调度规则。
- macOS/Windows XIASS Codex 配置助手，支持自动备份、历史会话修复、安全重启与恢复。
- OpenWrt/PassWall SOCKS 节点上报、FRP 反向连接和公网认证代理。
- GitHub Release 检查、Docker 在线更新、完整备份与跨服务器恢复。

## 批量授权与号池

1. 在账号管理打开“号池管理”，填写自定义名称和代理节点，创建号池。已有账号可多选加入；加入后继承号池代理。
2. 打开“批量添加账号”，按每行 `邮箱----密码----2FA密钥` 粘贴自己的已有账号。选择号池、业务分组、并发数、优先级和指纹模式，默认关闭指纹收敛。
3. 点击“开始授权”。每个账号使用独立 OAuth state 和独立隐私上下文，最多三个并行。普通或远程代理保持原代理；当前执行节点的 XIASS 内置回环出口在浏览器容器中使用同机等价出口，账号仍保存原代理 ID，不改变客户调度。
4. 进入手机号节点后自动使用独立接码会话。号码被拒绝时自动换号；短信等待满 180 秒后自动取消旧号码，并以新的 OAuth state 和隐私上下文从头重试。
5. 每条任务按打开窗口、邮箱、密码、2FA、手机号、短信、工作空间、回调和保存九个阶段实时显示；常规自动操作之间保留约一秒间隔，回调等待持续刷新。
6. CAPTCHA、邮件验证码、账号限制或异常页面会先关闭临时上下文并自动重试一次；最终结果汇总成功与失败账号，失败项可单选、多选或全选后重新授权，超过三个时自动排队续接。
7. 授权成功后读取并核验实际账号配置；保留邮箱，密码和 2FA 密钥通过专用加密字段保存。401 时可复用原“一键重新授权”，更新原账号，不另建账号。密码和 2FA 密钥不进入普通账号列表、审计正文或导出文件。

号池只负责整理账号与批量设置代理，不替代面向用户的计费分组。移出或删除号池保留账号当前代理。未完成的批量任务不跨服务重启续跑；已成功保存的账号和加密登录信息不受影响。遇到人机验证、邮件验证或上游明确封禁时停止并交由管理员处理，不绕过上游限制。

公共令牌转换页中的账号解析与实时 2FA 工具只在浏览器本地计算，不上传或保存输入；它与管理员主动提交的批量授权流程分别处理。
...
2. 默认填入 `https://api.xiass.com`，也可改为当前使用的 XIASS 站点地址；随后打开已登录页面并只显示当前登录账号名下、绑定到 OpenAI 分组的有效密钥。未登录用户无法读取任何密钥。
3. 先完整退出 Codex，再创建原配置的完整备份与 SHA-256 校验记录。
4. 切换 model provider、恢复旧版 `xiass` 配置或手动执行历史修复时，自动扫描普通会话和归档会话，并兼容旧版 `~/.codex/state_5.sqlite` 与新版 `~/.codex/sqlite/` 会话数据库；同一 provider 下仅调整模型、上下文或密钥时保留会话和索引，跳过全量扫描。
5. 修改历史可见性前由 SQLite 生成包含已提交 WAL 数据的一致性快照并记录 SHA-256，另行备份原始会话元数据、`session_index.jsonl`、桌面状态和工作区映射；会话正文不会被删除。
6. 保留已有推理强度、MCP、插件、项目和桌面设置，仅更新 XIASS Provider 必需字段；已有自定义 Provider 会沿用原标识，新配置和旧版 `xiass` 使用稳定的 `codex_local_access`，避免无谓改名后隐藏旧会话。
7. 将普通会话、归档会话和兼容数据库中的旧 Provider 标识统一到当前 Provider；同时校验项目路径与 `rootPaths`，修复会导致侧栏隐藏会话的异常工作区映射；原子写入会话元数据并使用 SQLite 事务同步索引，随后检查数据库完整性、Provider 一致性、精确线程 ID 集合、rollout 文件集合和项目映射。
8. 修复清单会持久记录准备、写入、提交和回滚状态；进程意外中断后，下次运行会先恢复未完成操作。任一步失败都会自动恢复配置和会话索引，只有全部校验成功才重新启动 Codex。
9. 助手页面提供“立即修复历史会话并重启 Codex”，也可以选择任意历史配置备份恢复；配置备份与已完成的历史会话备份均可单独删除，不会关闭或重启 Codex。
10. 助手页面可在配置前选择兼容档、当前默认、512K、1M/900K 或自定义上下文，并将 `model_context_window` 与 `model_auto_compact_token_limit` 一起写入本机配置；网站选密钥跳转期间也会保留这次选择。

助手还提供独立的手动配置入口，可填写任意兼容 Responses API 的 Base URL、API 密钥、默认会话模型和可选审查模型。可在本机读取该服务的 `/v1/models` 目录后分别选择，也可在目录缺失时手动填写；目录不会成为硬编码或持久化的模型白名单。公网地址必须使用 HTTPS；本机 `localhost`、`127.0.0.0/8` 和 `::1` 映射可使用 HTTP，并保留用户填写的 `/v1`、`/V1` 等路径。手动填写的密钥只提交到随机的 `127.0.0.1` 本机端口，不会发送给 XIASS API。网站选择的密钥通过回环地址的 URL Fragment 交给助手，不会进入 Nginx 或浏览器请求日志。
