<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/当前版本-v1.0.67-0ea5e9" alt="当前版本 v1.0.67" />
...
7. 拉取 `ghcr.io/xyf0104/nowind-api:latest` 并启动完整容器栈。
8. 等待数据库迁移完成并验证健康接口。
...
该脚本会先停止容器并创建一致性完整备份，再同步部署文件、拉取镜像、重建应用并执行健康检查。首次从 v1.0.65 更新时会安全停止旧栈并迁移运行时名称；失败时会尽量恢复原 Git 状态与旧栈。数据不会被删除。
...
- Release 与 `ghcr.io/xyf0104/nowind-api:<version>`、`latest` 同步发布。
