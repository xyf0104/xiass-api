...
export interface User {
  ...
  allowed_groups: number[] | null // Exclusive grants, plus selected public groups when restriction is enabled
  restrict_public_groups?: boolean // false/omitted keeps all public groups available
...
export interface Group {
  ...
  // Codex 网页搜索单次价格（人民币/次）；null 表示使用默认价 0.01
...
export interface AdminGroup extends Group {
  ...
  model_routing_pools?: Record<string, number[]> | null
...
export interface UsageProgress {
  ...
  weekly_estimate_usd?: number | null // 进入整数百分比时冻结账号已用，并按上一个百分点推算的 7d 额度
...
export interface AdminDataPayload {
  ...
  // 导出时被排除的关联凭据账号数量；字段名为兼容旧版继续沿用 skipped_shadows。
