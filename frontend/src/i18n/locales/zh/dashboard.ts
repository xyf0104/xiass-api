    ...
    useKeyModal: {
      ...
      openai: {
        ...
        helper: {
          ...
          hint: '点击下载时会自动复制当前站点地址；打开助手后粘贴该地址即可选择自己的密钥。首次运行若系统拦截，请在系统提示中确认仍要打开。'
        }
      },
      cliTabs: {
      ...
      grok: {
        description: '配置 Grok Build、Claude Code、Codex 或 OpenCode，让请求通过当前 XIASS API Grok 分组发送。',
        claudeDescription: '配置 Claude Code，通过当前 XIASS API Grok 分组调用 Grok 的 Anthropic Messages 兼容接口。',
        ...
        claudeNote: '这些环境变量仅在当前终端会话中生效；如需长期使用，请合并到终端配置和 ~/.claude/settings.json。',
