export default {
    executionNodes: {
      ...
      removeNode: '移除节点',
      ingressTitle: '客户怎么访问',
      ingressDescription: '客户继续使用原来的地址和 Key。这里的比例只决定两台机器如何使用账号；如果还要让两个域名都能访问，需要另外给域名配置健康切换，本页不会自动修改域名。',
      canEnable: '检查通过，可以打开',
      cannotEnable: '还不能打开',
      ...
      sharedAccess: {
        ...
        takeoverTitle: '已进入临时接管',
        takeoverDescription: '主机器已明确离线且本机已开启接管，现在可以暂时管理共享配置和对方账号；主机器恢复后会自动回到只读。',
      ...
      issues: {
        ...
        EMERGENCY_EGRESS_SETTING_INVALID: '临时接管设置无效，请重新保存。',
