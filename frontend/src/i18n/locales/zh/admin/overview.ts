export default {
    ...
    users: {
      ...
      today: '今日',
      total: '近30天',
      usageScopeHint: '该用户在全部分组中的今日与近 30 天扣费。',
      concurrencyScopeHint: '当前进入系统且尚未结束的请求 / 用户并发上限；包含选号、等待和故障转移阶段。',
      sortBy: '排序方式',
    ...
    groups: {
      ...
      searchGroups: '搜索分组...',
      viewActiveConcurrencyAccounts: '查看本分组当前请求和调用账号',
      createGroup: '创建分组',
      editGroup: '编辑分组',
      deleteGroup: '删除分组',
      duplicate: '复制',
      ...
      usageToday: '今日',
      usageTotal: '累计',
      usageScopeHint: '本分组所有用户产生的今日与累计扣费，不等于当前调用用户的费用合计。',
      ...
      userAccountAllowlist: {
        ...
        searchPlaceholder: '搜索当前分组用户...',
        chooseGroup: '选择要查看当前调用用户并配置的分组。',
        noAccountGroups: '该账号当前未关联可配置分组。',
        noRuntimeUsers: '当前没有用户正在使用此分组。',
        noUsersForAccount: '当前没有用户正在使用所选账号。',
        noSearchResults: '当前分组没有匹配的用户。',
        user: '用户',
        currentAccounts: '当前调用账号',
        currentConcurrency: '当前请求',
        currentConcurrencyHint: '该用户在本分组内当前尚未结束的请求；账号后的数量是已占槽请求，等待账号槽位显示尚未取得账号槽位的请求。所有用户之和与分组当前值一致。',
        requests: '个请求',
        ...
        unavailableSelected: '另有 {count} 个已选账号暂不可用',
        noSchedulableAccounts: '该分组暂无可配置账号',
        allow: '允许',
        account: '账号',
        status: '状态',
        ...
        unavailable: '暂不可用',
        allowed: '已允许',
        denyAllAccounts: '保存后，该用户将无法调用此分组中的任何账号。',
      ...
      form: {
        ...
        maxReasoningEffortHint: '限制客户端主动请求的 reasoning effort；Composite 分组仅对解析到 OpenAI 的请求生效。不会为缺省请求主动开启推理。上限优先级高于推理强度映射。',
        ...
        reasoningEffortMappingsHint: '可按模型精确、前缀或后缀匹配，并为同一范围配置多个请求值。留空模型表示匹配全部模型。',
        ...
        reasoningEffortModelPlaceholder: '留空表示全部模型',
      ...
      videoPricing: {
        title: '视频生成计费',
        description:
          '配置 Grok 视频生成的每秒单价，留空则使用默认每秒价（grok-imagine-video：480p ¥0.05/s、720p ¥0.07/s；video-1.5：480p ¥0.08/s、720p ¥0.14/s、1080p ¥0.25/s）',
      ...
      explicitPricing: {
        ...
        description: '分组级 web_search（每千次）与 Voice realtime / TTS / STT 单价（人民币）。留空表示未配置。',
        searchPricePer1k: '搜索每千次价格（¥）',
        pricePlaceholder: '可选'
      },
      modelPricing: {
        title: '分组逐模型定价',
      ...
      voicePricing: {
        title: 'Grok Voice 定价',
        description: '分组级 Voice realtime / TTS / STT 单价（人民币）。留空表示未配置。',
        audioRealtimePerMin: 'Realtime 每分钟价格（¥）',
      ...
      webSearchPricing: {
        title: 'Codex 网页搜索计费',
        pricePerCall: '搜索单次价格（¥/次）',
        pricePerCallHint:
          '留空使用默认基础价 ¥0.01/次；填 0 表示免费。实际扣费会叠加分组费率倍数。',
      ...
      openaiLive: {
        title: 'OpenAI Live',
        allow: '允许访问 Live',
        hint: '启用后，此 OpenAI 分组的 API Key 可以创建并控制 Live 语音会话。默认关闭。运行 XIASS API 的服务端必须是 Apple Silicon Mac，并安装官方 ChatGPT App；客户端平台不受限制。',
        unsupportedTitle: '当前服务端不支持 Live',
        unsupportedMessage: '当前 XIASS API 服务端无法生成 Live 所需的设备证明，即使开启也不能使用。是否仍然开启？',
        enableAnyway: '仍然开启'
      },
