export default {
  ...
  usage: {
    ...
    weeklyEstimate: '周额度约',
    weeklyEstimateHint: '从真实 0% 加入或周额度重置后保留规则 B：首次观察到跳至 N% 时，以对齐的累计账号已用 ÷ ((N−1)/100) 估算，N≤1 时统计中，同一档内金额不浮动重算。中途加入使用（当前对齐账号已用−加入时账号已用）÷（当前实际百分比−加入实际百分比）×100；至少完成一个实际百分点后更新，未完成下一百分点时保留上一结果。例如 20%/$0 到 25%/$120 为 $2,400。无法核实加入点时不猜测为 0%；明确的站外消耗不计入本地分母。同一 ChatGPT 身份 401 重新授权保留已核实记录；100% 时显示实际账号已用。',
    ...
    endTime: '结束时间',
    applyTimeRange: '查询',
    invalidMinuteRange: '请选择有效的分钟级时间范围，开始时间必须早于结束时间',
    requestCount: '请求次数',
    modelBillingDetails: '模型扣费明细',
    backToUsers: '返回用户列表',
    user: '用户',
    noBillingDetails: '所选时间范围内暂无扣费记录',
    billingDetailsFailed: '读取扣费明细失败',
    resetNow: '现在',
    resetPending: '待刷新',
  ...
  admin: {
    executionNodes: {
      title: '双节点负载均衡',
      description: '统一账号池调度与节点固定出口配置。默认关闭，完成共享状态和节点出口预检后才能启用。',
      pairingTitle: '网页配对',
      pairingDescription: '通过一次性邀请码确认对端身份，并校验两台实例是否共用 PostgreSQL、Redis 与鉴权密钥。配对不会传输账号凭据或数据库密码。',
      pairingReady: '生产集群已就绪',
      pairingIncomplete: '已记录对端，状态未完全一致',
      pairingNotPaired: '尚未配对',
      inviteTitle: '在本机生成邀请码',
      inviteHint: '邀请码 10 分钟内有效、只能使用一次。请通过安全渠道交给另一台 XIASS 的管理员。',
      generateInvite: '生成邀请码',
      regenerateInvite: '重新生成',
      copyInvite: '复制',
      inviteExpiry: '有效期至 {time}；关闭页面后不会再次显示原始邀请码。',
      noInvite: '尚未生成邀请码。生成操作不会修改调度或客户流量。',
      joinTitle: '连接另一台 XIASS',
      joinHint: '填写对端 XIASS 的 HTTPS 地址和对端生成的一次性邀请码。',
      peerURL: '对端地址',
      inviteToken: '一次性邀请码',
      pair: '开始配对',
      pairing: '配对中...',
      ...
      passed: '一致',
      failed: '不一致',
      pairingReadyHint: '两台实例读取同一账号、Key、余额、账单和心跳状态；账号管理会显示完整共享账号池及各账号的节点归属。',
      pairingMismatchHint: '当前只能证明两台实例可以通信。请先让两台实例连接同一套 PostgreSQL、Redis 和鉴权密钥，再重新配对；系统不会把两套独立账务强行合并。',
      pairingLoadFailed: '配对状态加载失败',
      pairingFailed: '节点配对失败',
      inviteGenerated: '一次性邀请码已生成',
      inviteCopied: '邀请码已复制',
      inviteCopyFailed: '无法复制邀请码',
      pairSuccess: '节点配对验证通过',
      unpairSuccess: '已解除本机配对记录',
      confirmUnpairTitle: '确认解除节点配对',
      confirmUnpairMessage: '解除配对只删除本机的对端记录，不删除账号、Key、账单或共享数据。建议先关闭多节点调度。',
      confirmUnpair: '确认解除',
      refresh: '刷新状态',
      saving: '保存中...',
      save: '保存配置',
      enabled: '启用双节点账号调度',
      enabledHint: '开启后，新会话会在当前最高账号优先级内按节点权重选择账号；已有粘性会话不会被强制迁移。',
      on: '已启用',
      off: '未启用',
      runtimeTitle: '本机运行状态',
      localNode: '本机节点',
      ...
      local: '本机',
      weight: '账号权重',
      weightHint: '权重只在当前最高账号优先级内生效；0 表示停止新会话进入该节点账号。',
      egress: '节点上游出口',
      egressHint: '账号归属哪个节点，正常情况下就通过该节点绑定的代理或公网出口访问上游；它不是客户使用的 Base URL，也不是公网入口权重。',
      proxyMissing: '未配置出口',
      proxyInvalid: '出口不可用',
      accounts: '账号',
      activeAccounts: '活跃',
      ...
      removeNode: '移除节点',
      ingressTitle: '公网入口说明',
      ingressDescription: '此面板控制共享账号池的节点权重，不改变客户请求先到哪台服务器。主域名与备用域名的公网分流需要在 Caddy、Cloudflare Load Balancing 或其他入口层配置，并使用 /readyz 做健康检查。',
      canEnable: '预检通过，可以启用',
      cannotEnable: '暂不能启用',
      confirmEnableTitle: '确认启用双节点调度',
      confirmEnableMessage: '启用前系统会为历史未归属账号补齐节点归属，并开始使用共享账号池。请确认两台机器已经连接同一套 PostgreSQL、Redis 和固定出口。',
      confirmEnable: '确认启用',
      issueTitle: '需要处理的问题',
      statusTitle: '当前状态',
      noIssues: '当前没有检测到配置问题。',
      reloadFailed: '负载均衡状态加载失败',
      saveSuccess: '负载均衡配置已保存',
      saveFailed: '负载均衡配置保存失败',
      invalidWeights: '节点 ID 必须唯一，权重必须是 0 到 1000000 的有限数字，且至少一个节点权重大于 0。',
      invalidProxy: '启用双节点调度时，每个节点都必须选择不同的有效固定出口。',
      issues: {
        MULTI_NODE_DISABLED: '多节点未启用，当前服务器按普通单机模式正常运行。',
        EXECUTION_NODE_LOCAL_CONFIG_DISABLED: '请先在本机部署配置中启用多节点运行参数，并重启 XIASS 后再开启账号调度。',
        EXECUTION_NODE_PROXY_MAPPING_INCOMPLETE: '节点固定出口映射不完整，请为每个节点配置独立且有效的固定出口。',
        EXECUTION_NODE_PROXY_MAPPING_DUPLICATE_PROXY: '节点固定出口不能重复使用，请为每个节点配置不同的固定出口。',
        EXECUTION_NODE_PROXY_MAPPING_UNKNOWN_NODE: '固定出口映射包含未知节点，请重新检查节点列表。',
        EXECUTION_NODE_PROXY_MAPPING_UNAVAILABLE: '当前已启用的节点固定出口映射无效，请先修复映射后再保存。',
        INVALID_EXECUTION_NODE_PROXY_IDS: '节点固定出口配置无效，请检查出口选择。',
        EXECUTION_NODE_LEGACY_PROXY_INVALID: '历史账号默认出口不存在、已停用或已过期。',
        EXECUTION_NODE_LOCAL_WEIGHT_MISSING: '共享权重中缺少当前服务器节点，请补充当前节点后再保存。',
        EXECUTION_NODE_LEGACY_WEIGHT_MISSING: '共享权重中缺少历史账号归属节点，请补充后再保存。',
        EXECUTION_NODE_LOCAL_PROXY_MISMATCH: '当前服务器的默认出口与共享映射不一致，请修正后再保存。',
        EXECUTION_NODE_LEGACY_PROXY_MISMATCH: '历史账号默认出口与共享映射不一致，请修正后再保存。',
        EXECUTION_NODE_PAIRING_NOT_READY: '两台 XIASS 尚未完成生产级配对和共享状态校验。',
        EXECUTION_NODE_PREFLIGHT_UNAVAILABLE: '节点账号预检服务当前不可用，请检查数据库、Redis 和账号仓库。',
        EXECUTION_NODE_PREFLIGHT_FAILED: '节点账号预检未通过，请检查账号归属、固定出口和共享状态。',
        EXECUTION_NODE_POLICY_UNAVAILABLE: '共享节点调度策略当前不可用，请稍后重试。',
        EXECUTION_NODE_PROXY_REQUIRED: '多节点调度下每个账号都必须保留固定出口。',
        EXECUTION_NODE_PROXY_IMMUTABLE: '多节点调度开启后不能直接修改账号固定出口。',
        DATABASE_UNAVAILABLE: '共享设置数据库当前不可用。',
        SHARED_WEIGHTS_INVALID: '共享节点权重配置无效。',
        SHARED_PROXY_MAPPING_INVALID: '共享节点与出口映射无效。',
        LOCAL_RUNTIME_DISABLED: '当前服务器尚未启用多节点运行参数，请先完成本机部署配置。',
        LOCAL_NODE_ID_INVALID: '当前服务器缺少有效的节点 ID。',
        LOCAL_DEFAULT_PROXY_MISSING: '当前服务器尚未配置默认固定出口。',
        LOCAL_WEIGHT_MISSING: '共享权重中没有当前服务器的节点 ID。',
        LOCAL_PROXY_MISMATCH: '共享出口映射与当前服务器的默认出口不一致。',
        LEGACY_OWNER_INVALID: '历史账号归属节点或历史出口配置不完整。',
        LEGACY_PROXY_MISMATCH: '历史账号出口映射与部署参数不一致。',
        POSITIVE_WEIGHT_MISSING: '至少需要一个权重大于 0 的节点。',
        PROXY_MAPPING_INVALID: '节点固定出口映射不完整或存在重复。',
        NODE_PROXY_UNAVAILABLE: '存在未绑定有效固定出口的节点。',
        NODE_OFFLINE: '存在心跳离线的执行节点。',
        HEARTBEAT_STORE_UNAVAILABLE: 'Redis 节点心跳当前不可用。',
        PROXY_STATUS_UNAVAILABLE: '无法读取固定出口状态。',
        PAIRING_NOT_READY: '尚未完成生产级节点配对或共享状态校验。',
        ACCOUNT_STATS_UNAVAILABLE: '无法读取节点账号统计。'
      }
    },
    // Dashboard
    dashboard: {
    ...
    users: {
      ...
      // 用户分组配置
      groupConfig: '用户分组配置',
      groupConfigHint: '为用户 {email} 配置可用分组和专属倍率',
      exclusiveGroups: '专属分组',
      publicGroups: '公开分组（可为当前用户隐藏）',
      defaultRate: '默认倍率',
      customRate: '专属倍率',
      useDefaultRate: '使用默认',
    ...
    groups: {
      ...
      userAccountAllowlist: {
        title: '用户可调用账号',
        runtimeTitle: '当前调用用户',
        runtimeAvailableAccounts: '当前分组 {count} 个正常可调度账号',
        chooseGroup: '选择要查看当前调用用户并配置的分组。',
        noAccountGroups: '该账号当前未关联可配置分组。',
        noRuntimeUsers: '当前没有用户正在使用此分组。',
        noUsersForAccount: '当前没有用户正在使用所选账号。',
        user: '用户',
        currentAccounts: '当前调用账号',
        ...
        noSchedulableAccounts: '该分组暂无正常可调度账号',
        allow: '允许',
        account: '账号',
        status: '状态',
        ...
        normal: '正常',
        allowed: '已允许',
        requireOneAccount: '至少保留一个账号；不允许保存为全部拒绝。',
        restoreOriginal: '恢复原版调度',
        configure: '配置账号',
        saved: '用户可调用账号已保存',
        restored: '已恢复原版分组调度',
        loadFailed: '加载用户可调用账号失败',
        saveFailed: '保存用户可调用账号失败',
        restoreFailed: '恢复原版分组调度失败'
      },
      form: {
        name: '名称',
        description: '描述',
      ...
      modelRouting: {
        title: '模型优先路由',
        tooltip:
          '配置特定模型优先使用指定账号；OpenAI 分组还可选择号池。偏好范围不可用时会自动回退普通调度。支持末尾 * 通配符。',
        enabled: '已启用',
        disabled: '已禁用',
        disabledHint: '启用后，配置的路由规则才会生效',
        addRule: '添加路由规则',
        modelPattern: '模型模式',
        modelPatternPlaceholder: 'gpt-5.6-luna 或 claude-opus-*',
        modelPatternHint: '支持末尾 * 通配符，如 gpt-5.6-*',
        accounts: '优先账号',
        selectAccounts: '选择账号',
        noAccounts: '此分组暂无账号',
        ...
        removeRule: '删除规则',
        noRules: '暂无路由规则',
        noRulesHint: '添加规则，为特定模型指定优先账号或号池',
        searchAccountPlaceholder: '搜索账号...',
        accountsHint: '可多选；范围内仍按账号优先级、健康和并发调度',
        pools: '优先号池',
        loadingPools: '正在加载号池...',
        noPools: '暂无号池，请先在账号管理中创建',
        poolsHint: '可多选；号池成员变化会自动生效'
      },
      mcpXml: {
        title: 'MCP XML 协议注入',
    ...
    riskControl: {
      ...
      blockedKeywordsPlaceholder: '每行输入一条规则，例如：\n逆向 && 网站\ncontains:敏感词',
      blockedKeywordsDescription: '匹配忽略大小写；命中后会按下方策略决定是否调用上游审计接口。',
      blockedKeywordsContextHint: '推荐用“词 A && 词 B”组合上下文拦截；单个词只匹配完整输入，确需包含匹配时请显式写 contains:词。',
    ...
    accounts: {
      ...
      usageWindowsHint: '“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号设定，并非 XIASS API 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在 XIASS API 端解除该限制。',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 XIASS API 实例的场景。',
