      ...
      >
        ...
        <template #cell-tokens="{ row }">
          ...
          <div v-if="isVideoUsage(row)" class="flex items-center gap-1.5">
            ...
            <span class="text-gray-400">({{ formatVideoMetadata(row) }})</span>
          </div>
          <!-- 图片生成请求（仅按次计费时显示图片格式） -->
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <template v-if="tooltipData && !isImageUsage(tooltipData) && !isVideoUsage(tooltipData) && (!tooltipData.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN)">
              <div v-if="tooltipData && tooltipData.input_tokens > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">${{ formatTokenPricePerMillion(tooltipData.input_cost, textInputTokens(tooltipData), { withCurrencySymbol: false }) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && hasImageInputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-fuchsia-300">${{ formatTokenPricePerMillion(tooltipData.image_input_cost ?? 0, tooltipData.image_input_tokens, { withCurrencySymbol: false }) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && tooltipData.output_cost > 0 && textOutputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-violet-300">${{ formatTokenPricePerMillion(tooltipData.output_cost, textOutputTokens(tooltipData), { withCurrencySymbol: false }) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && hasImageOutputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-pink-300">${{ formatTokenPricePerMillion(tooltipData.image_output_cost ?? 0, tooltipData.image_output_tokens, { withCurrencySymbol: false }) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
            </template>
            <template v-else-if="tooltipData && isVideoUsage(tooltipData)">
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-white">{{ tooltipData.video_resolution || t('usage.unknown') }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">${{ videoSecondUnitPrice(tooltipData).toFixed(6) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
...
const formatVideoMetadata = (row: AdminUsageLog): string => {
  const resolution = row.video_resolution || t('usage.unknown')
  const duration = row.video_duration_seconds == null ? t('usage.unknown') : `${row.video_duration_seconds}s`
