...
const helperReleaseBase = 'https://github.com/xyf0104/xiass-api/releases/download/codex-helper-latest'
...
default = "xiass-grok"
web_search = "xiass-grok"

[model."xiass-grok"]
model = "grok-4.5"
base_url = "${baseUrl}"
name = "Grok 4.5 via XIASS API"
description = "Grok 4.5 through an XIASS API Grok group"
api_key = "${apiKey}"
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const configPath = isWindows
    ...
    : '~/.codex/config.toml'
  const configContent = `model_provider = "xiass_grok"
model = "grok-4.5"
review_model = "grok-4.5"
model_reasoning_effort = "high"
model_context_window = 1000000

[model_providers.xiass_grok]
name = "XIASS Grok"
base_url = "${baseUrl}"
env_key = "XIASS_API_KEY"
wire_api = "responses"
...
responses_websockets_v2 = true`
  const environmentContent = isWindows
    ? `$env:XIASS_API_KEY="${apiKey}"`
    : `export XIASS_API_KEY="${apiKey}"`
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    provider[platform].name = 'Grok via XIASS API'
