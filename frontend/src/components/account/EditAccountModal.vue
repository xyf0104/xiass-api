    ...
    >
      ...
      >
        <div class="space-y-3">
          ...
          <div
            v-else-if="grokMediaEligibilityState"
            class="rounded-lg bg-gray-50 p-3 text-xs dark:bg-dark-700"
          >
...
const loadGrokMediaEligibility = async (
  accountID: number
): Promise<GrokMediaEligibilityState | null> => {
  if (
    !isGrokOAuthAccount.value ||
    typeof adminAPI.accounts.getGrokMediaEligibility !== 'function'
  ) {
    return null
  }
  ...
  try {
    ...
    if (requestVersion !== grokMediaEligibilityRequestVersion) return null
    grokMediaEligibilityError.value =
      error?.message || t('admin.accounts.grokMediaEligibility.loadFailed')
    return null
  } finally {
...
const persistGrokMediaEligibility = async (
  accountID: number,
  ...
  try {
    const state = await adminAPI.accounts.updateGrokMediaEligibility(
      accountID,
...
const submitUpdateAccount = async (accountID: number, updatePayload: Record<string, unknown>) => {
  ...
  try {
    let updatedAccount = await adminAPI.accounts.update(
      accountID,
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      if (props.account.platform === 'grok') {
        const validationError = validateGrokBaseUrlInput(newBaseUrl)
        if (validationError) {
    ...
    if (props.account.platform === 'grok' && props.account.type === 'oauth') {
      ...
      if (grokOAuthCustomBaseUrlEnabled.value) {
        ...
        newCredentials.base_url = grokOAuthBaseUrl.value.trim()
        newCredentials[GROK_CUSTOM_BASE_URL_ENABLED_CREDENTIAL_KEY] = true
      } else if (grokOAuthCustomBaseUrlWasEnabled.value) {
        delete newCredentials.base_url
        delete newCredentials[GROK_CUSTOM_BASE_URL_ENABLED_CREDENTIAL_KEY]
      }
