...
describe('EditAccountModal Grok OAuth upstream config', () => {
  ...
  it('loads an existing custom base_url with the toggle on and keeps it on save', async () => {
    ...
    const payload = updateAccountMock.mock.calls[0]?.[1]
    expect(payload?.credentials?.base_url).toBe('https://my-relay.example.com')
    expect(payload?.credentials?.grok_custom_base_url_enabled).toBe(true)
  })

  it('keeps an unmarked legacy base_url disabled and preserves it on an untouched save', async () => {
    const account = buildGrokOAuthAccount({ base_url: 'https://legacy-relay.example.com' })
    updateAccountMock.mockResolvedValue(account)
    ...
    const payload = updateAccountMock.mock.calls[0]?.[1]
    expect(payload?.credentials?.base_url).toBe('https://legacy-relay.example.com')
    expect(payload?.credentials?.grok_custom_base_url_enabled).toBeUndefined()
  })
