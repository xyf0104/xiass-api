...
const handleEscape = (event: KeyboardEvent) => {
  if (
    ...
    props.closeOnEscape &&
    event.key === 'Escape' &&
    isTopmostDialog(dialogInstanceId)
  ) {
    emit('close')
  }
}
...
watch(
  ...
  async (isOpen) => {
    if (isOpen) {
      ...
      registerOpenDialog(dialogInstanceId, props.zIndex)

      // 等待DOM更新后设置焦点到对话框
      await nextTick()
      if (props.show && isTopmostDialog(dialogInstanceId) && dialogRef.value) {
      ...
      const shouldRestoreFocus = isTopmostDialog(dialogInstanceId)
      unregisterOpenDialog(dialogInstanceId)
      // 恢复之前的焦点
      if (
        shouldRestoreFocus &&
...
watch(
  () => props.zIndex,
  (zIndex) => {
    if (props.show) registerOpenDialog(dialogInstanceId, zIndex)
  }
)
