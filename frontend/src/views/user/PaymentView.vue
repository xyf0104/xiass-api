  ...
  <AppLayout>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showRenewalModal" data-test="renewal-modal" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/60 p-4 backdrop-blur-sm" @click.self="closeRenewalModal">
          <div data-test="renewal-modal-panel" class="relative max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-dark-700 dark:bg-dark-900">
            <!-- Close button -->
