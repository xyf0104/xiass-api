        ...
        <div class="border-t pt-4">
          ...
          >
            ...
            <div class="border-t border-gray-200 px-3 py-2 dark:border-dark-600">
              ...
              <p v-if="createAllowlistCustomErrorKey" class="mt-1 text-xs text-red-500">
        ...
        >
          ...
          >
            ...
            <div class="mt-3 space-y-3">
              ...
              >
                <div
                  class="min-w-0 pb-1 font-mono text-xs text-gray-700 dark:text-gray-300"
                >
                  {{ family.label }}
                </div>
                <label
        ...
        <div class="border-t pt-4">
          ...
          >
            ...
            <div class="border-t border-gray-200 px-3 py-2 dark:border-dark-600">
              ...
              <p v-if="editAllowlistCustomErrorKey" class="mt-1 text-xs text-red-500">
...
const submitCreateAllowlistCustomEntry = () => {
  ...
  createAllowlistCustomErrorKey.value = error
    ? `admin.groups.modelAllowlist.errors.${error}`
    : null;
  if (!error) createAllowlistCustomEntry.value = "";
};
...
const submitEditAllowlistCustomEntry = () => {
  ...
  editAllowlistCustomErrorKey.value = error
    ? `admin.groups.modelAllowlist.errors.${error}`
    : null;
  if (!error) editAllowlistCustomEntry.value = "";
};
...
// Codex 网页搜索单次默认价（与后端 defaultWebSearchPricePerCall 一致）
...
const handleCreateGroup = async () => {
  ...
  try {
    const {
      video_model_prices: _videoModelPricesForm,
      ...createGroupForm
    } = createForm;
    ...
    const requestData = {
      ...
      // 创建分组时固定账号 manifest 由编辑流程显式开启，避免新分组误用旧账号。
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    // 可空数值输入清空时 v-model.number 产生 ""，直接提交会被后端 *float64 反序列化拒绝（400）。
    // 更新语义中 null 表示"不修改"，因此清空后的字段发送 -1，由后端归一为 NULL。
    ...
    payload.cost_ratio = emptyPriceToClear(payload.cost_ratio);
