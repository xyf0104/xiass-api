...
//
// /v1/usage、/v1/sub2api/billing、模型列表端点与异步生图任务查询只需鉴权，不需要计费执行。
// usage 允许过期/配额耗尽的 Key 查询自身用量，billing 用于读取当前 Key 的倍率配置，
// 模型列表供兼容客户端读取当前分组模型，异步生图查询允许已耗尽额度的 Key 拉取自身任务结果。
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		skipBilling := c.Request.URL.Path == "/v1/usage" || billingInfoRequest || isModelCatalogRead(c.Request.Method, c.Request.URL.Path) || isAsyncImageTaskRead(c.Request.Method, c.Request.URL.Path)
...
func isModelCatalogRead(method, path string) bool {
	return method == http.MethodGet && (path == "/v1/models" || path == "/models")
}
