...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers, settingService *service.SettingService) {
	...
	groups.Use(middleware.ExecutionNodeSharedWriteGuard(settingService))
	{
		...
		groups.GET("/:id/user-account-runtime", h.Admin.Group.GetUserAccountRuntime)
		groups.GET("/:id/users/:user_id/account-allowlist", h.Admin.Group.GetUserAccountAllowlist)
		groups.PUT("/:id/users/:user_id/account-allowlist", h.Admin.Group.ReplaceUserAccountAllowlist)
		groups.DELETE("/:id/users/:user_id/account-allowlist", h.Admin.Group.DeleteUserAccountAllowlist)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.GET("/:id/billing-breakdown", h.Admin.Account.GetBillingBreakdown)
...
func registerOpenAIOAuthRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	openai := admin.Group("/openai")
	{
		...
		openai.POST("/batch-oauth/tasks/:task_id/adspower-launch", h.Admin.OpenAIOAuth.LaunchBatchOAuthTaskInAdsPower)
		openai.POST("/reauthorization/tasks/:task_id/adspower-launch", h.Admin.OpenAIOAuth.LaunchOpenAIReauthorizationTaskInAdsPower)
		...
		openai.POST("/team-child/members/invite", h.Admin.OpenAIOAuth.InviteTeamChildMember)
		...
		openai.POST("/team-child/workflows/:workflow_id/pause", h.Admin.OpenAIOAuth.PauseTeamChildWorkflow)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers, settingService *service.SettingService) {
	adminSettings := admin.Group("/settings")
	{
		...
		// XIASS server-side OAuth SMS receiver. Card keys remain encrypted at
		// rest; plaintext is returned only by the administrator-only queue API.
		...
		adminSettings.GET("/sms-receiver/card-keys", h.Admin.Setting.ListPixlabSMSCardKeys)
		...
		adminSettings.DELETE("/sms-receiver/card-keys/:card_key_id", h.Admin.Setting.DeletePixlabSMSCardKey)
		adminSettings.PUT("/sms-receiver/card-keys/order", h.Admin.Setting.ReorderPixlabSMSCardKeys)
