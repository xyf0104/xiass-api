...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	// CN Anthropic accounts need protocol conversion to their native endpoint;
	...
	if instructionsEmpty && account.Type == AccountTypeOAuth && !responsesLite && !compatMessagesBridge && !nativeCNResponses {
	...
	// Compact requests, including remote compaction v2 carried by a normal
	// /responses body, must not receive image bridge tools or instructions.
	skipImageGenerationBridge := isCompactRequest || HasCompactionTriggerInInput(body)
	if imageGenerationAllowed && !skipImageGenerationBridge && (codexImageGenerationBridgeEnabled || isOpenAIImageGenerationModel(requestView.Model) || openAIRequestBodyImageGenerationToolNeedsNormalization(body) || isOpenAIImageGenerationModel(upstreamModel)) {
		decoded, decodeErr := ensureReqBody()
		if decodeErr != nil {
			return nil, decodeErr
		}
	...
	if account.Type == AccountTypeOAuth {
		...
		if codexResult.Error != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": gin.H{"type": "invalid_request_error", "message": codexResult.Error.Error(), "param": "tools"}})
		...
		// Account identity isolation and optional fingerprint convergence are
		// independent. Scope client metadata to the OAuth credential first; the
		// explicit XIASS fingerprint mode may then converge the staged values.
	...
	// Remember the account-scoped entry identity before any recovery retry
	// rewrites the request. A cache miss (node switch or process restart) is
	// harmless and falls back to the existing one-shot recovery below.
	...
	if wsDecision.Transport == OpenAIUpstreamTransportResponsesWebsocketV2 {
		...
		recoverInvalidEncryptedContent := func(attempt int) bool {
			...
			if len(invalidDigests) > 0 {
				...
				s.markOpenAIWSInvalidEncryptedContentLineage(lineageGroupID, account.ID, lineageSessionHash, invalidDigests)
			}
			previousResponseID := openAIWSPayloadString(wsReqBody, "previous_response_id")
...
func shouldForwardOpenAIResponsesViaRawChatCompletions(account *Account) bool {
	...
	if account.IsCNProvider() {
		// CN 的显式协议配置优先于异步探针 Extra；adaptive 仅具备原生 Responses
		// 的平台直转，其余回退 Chat Completions。
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	// DeepSeek / Kimi native Responses endpoints are stateless.
