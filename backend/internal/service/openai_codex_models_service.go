...
// CodexModelsManifest is retained for XIASS callers compiled against the
	...
	if isOpenAICodexGPTModel(modelID) {
		...
		if isOpenAICodexReasoningGPTModel(modelID) {
			...
			if isOpenAIGPT6Model(modelID) {
				...
				// Keep the GPT-6 family on the same large-context contract.
				...
				if isOpenAIGPT6AstraModel(modelID) {
					// Only Astra advertises the multi-agent workflow. Sol/Luna share
					// the GPT-6 context contract without inheriting that metadata.
...
// may succeed without changing the request. Permanent endpoint/status errors
// must stay attached to the selected account: retrying them can silently mix
// account capabilities and makes a deterministic API-Key failure look like a
// scheduler problem. Only rate limits, server failures, and the OAuth backend
// 401 case are eligible for failover.
...
func isRetryableCodexModelsManifestStatus(statusCode int, useAPIKeyUpstream bool) bool {
	return (statusCode == http.StatusUnauthorized && !useAPIKeyUpstream) ||
...
func (c *openAIModelsCache) get(key string, now time.Time) (*OpenAIModelsResponse, openAIModelsCacheState) {
	...
	if now.Before(entry.expiresAt) {
		return cloneOpenAIModelsResponse(entry.manifest), openAIModelsCacheFresh
	}
	return cloneOpenAIModelsResponse(entry.manifest), openAIModelsCacheStale
}
...
func (c *openAIModelsCache) set(key string, manifest *OpenAIModelsResponse, now time.Time) {
	...
	c.entries[key] = openAIModelsCacheEntry{
		manifest:   cloneOpenAIModelsResponse(manifest),
		order:      c.nextOrder,
...
func (s *OpenAIGatewayService) FetchCodexModelsManifest(ctx context.Context, account *Account, clientVersion, ifNoneMatch string) (*OpenAIModelsResponse, error) {
	...
	case credAccount.IsOpenAIApiKey():
		...
		if isOfficialOpenAIModelsBaseURL(baseURL) {
			return nil, infraerrors.New(
				http.StatusBadGateway,
				"OPENAI_CODEX_MODELS_API_KEY_UPSTREAM_UNSUPPORTED",
				"official OpenAI API keys do not expose the Codex models manifest",
			)
		}
...
func (s *OpenAIGatewayService) fetchCachedOpenAIModels(ctx context.Context, request openAIModelsRequest, fetch func(ctx context.Context, ifNoneMatch string) (*OpenAIModelsResponse, error), ifNoneMatch string) (*OpenAIModelsResponse, error) {
	...
	resultCh := s.refreshCachedOpenAIModels(cacheKey, request, fetch, ifNoneMatch)
...
func (s *OpenAIGatewayService) refreshCachedOpenAIModels(cacheKey string, request openAIModelsRequest, fetch func(ctx context.Context, ifNoneMatch string) (*OpenAIModelsResponse, error), clientIfNoneMatch string) <-chan singleflight.Result {
	return s.openAIModelsCache.refresh.DoChan(cacheKey, func() (any, error) {
		...
		if cached != nil {
			...
			ifNoneMatch = strings.TrimSpace(clientIfNoneMatch)
		}
...
func (s *OpenAIGatewayService) fetchOpenAIModelsUpstream(ctx context.Context, request openAIModelsRequest, ifNoneMatch string) (*OpenAIModelsResponse, error) {
	...
	if int64(len(body)) > codexModelsManifestBodyLimit {
		return nil, &codexModelsManifestUpstreamError{
			err:       infraerrors.Newf(http.StatusBadGateway, "OPENAI_CODEX_MODELS_UPSTREAM_FAILED", "codex models manifest response exceeds %d bytes", codexModelsManifestBodyLimit),
			retryable: true,
		}
	}
