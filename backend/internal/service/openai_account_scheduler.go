...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if s.service != nil && account.IsGrok() {
		upstreamModel = s.service.canonicalGrokSchedulingModel(ctx, account, req.RequestedModel)
	}
	now := time.Now()
	...
	buildNodeSelectionOrder := func(pool []openAIAccountCandidateScore) []openAIAccountCandidateScore {
		...
		if !openAIRequestHasStickyAnchor(req) {
			if openAICandidatesHaveDistinctLastUsed(pool) {
				newSessionRanked := append([]openAIAccountCandidateScore(nil), pool...)
				sort.SliceStable(newSessionRanked, func(i, j int) bool {
					return isOpenAINewSessionCandidateBetter(newSessionRanked[i], newSessionRanked[j])
				})
				primary = append(primary, newSessionRanked[:groupTopK]...)
			} else {
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		selection, selectErr := s.service.newSelectionResultFromAccount(ctx, fresh, true, result.ReleaseFunc, nil)
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		// Weighted sticky is only a preference inside the current priority tier;
		// it must not reintroduce a Grok account removed by the normal quota gates.
		if len(s.filterGrokFreeQuotaAccounts(ctx, []Account{*account})) == 0 {
			continue
		}
		upstreamModel := canonicalOpenAIAccountSchedulingModel(account, req.RequestedModel)
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	if normalizeOpenAICompatiblePlatform(req.Platform) == PlatformGrok {
		...
		accounts = filterGrokModelQuotaBlockedAccounts(filtered, req.RequestedModel, now)
		if len(accounts) == 0 {
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if req.RequestedModel != "" {
		supported := account.IsModelSupported(req.RequestedModel)
		if s.service != nil {
			supported = s.service.isGrokRequestModelSupported(ctx, account, req.RequestedModel)
		}
		if !supported {
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	if scheduler == nil {
		...
		if previousResponseID != "" && platform == PlatformOpenAI && !previousResponseCanMove {
