...
func TestOpenAIModelResponseMismatchGuardHTTPMessageAndUnknownBody(t *testing.T) {
	...
	_, mismatch = guard.mismatch("provider/gpt-6-luna-2026-09-01", true)
	require.True(t, mismatch, "a terminal Luna declaration must win over an earlier conflicting event")
	require.Contains(t, openAIModelResponseMismatchMessage("gpt-6-luna"), "luna模型")
