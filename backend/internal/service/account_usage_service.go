...
// getUsageForAccount queries usage for an already-loaded account. It keeps the
// existing XIASS platform-specific paths intact while allowing batch callers to
// avoid an additional account repository lookup.
...
// GetAccountWindowStatsBatch returns window statistics for accounts sharing the
// same start time. Account-list callers use it to avoid one aggregation query
// per account while preserving the existing single-account fallback behavior.
func (s *AccountUsageService) GetAccountWindowStatsBatch(ctx context.Context, accountIDs []int64, startTime time.Time) (map[int64]*usagestats.AccountStats, error) {
	...
	if batchReader, ok := s.usageLogRepo.(accountWindowStatsBatchReader); ok {
		...
		if err == nil {
			for _, accountID := range uniqueIDs {
				if statsByAccount[accountID] == nil {
					result[accountID] = &usagestats.AccountStats{}
					continue
				}
				result[accountID] = statsByAccount[accountID]
			}
			return result, nil
		}
	}
...
func parseOpenAIWeeklyEstimateBool(raw map[string]any, key string) (bool, bool) {
	value, ok := raw[key]
	if !ok || value == nil {
		return false, false
	}
...
func parseOpenAIWeeklyEstimateNumber(raw map[string]any, key string) (float64, bool) {
	...
	return parseOpenAIWeeklyEstimateValue(value)
}

func parseOpenAIWeeklyEstimateValue(value any) (float64, bool) {
	var parsed float64
	switch typed := value.(type) {
	case float64:
		parsed = typed
...
func sameOpenAIWeeklyEstimateWindow(previous, current, now time.Time) bool {
	if previous.IsZero() || current.IsZero() {
		return previous.IsZero() && current.IsZero()
	}
	// The upstream gives a relative reset duration, and each later snapshot
	// derives a new absolute timestamp from that duration. That timestamp is a
	// moving ETA, not a stable window ID. While both observations still describe
	// an active window, monotonic percent/cost checks are the reliable boundary.
	// A whole-day movement is not a normal ETA revision for a seven-day window,
	// so retain a bound that still recognizes an early/new window transition.
	return previous.After(now) && current.After(now) &&
		previous.Sub(current).Abs() <= openAIWeeklyEstimateMaxActiveResetDrift
}
