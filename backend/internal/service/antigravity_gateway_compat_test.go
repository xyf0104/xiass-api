...
func TestAdaptAntigravityResponsesRequestPreservesClientToolMapping(t *testing.T) {
	body := []byte(`{
	...
	request, adapted, mapping, err := adaptAntigravityResponsesRequest(body)

	require.NoError(t, err)
	require.Equal(t, "gemini-3.1-pro-high", request.Model)
	...
	require.Equal(t, "tool_search", gjson.GetBytes(adapted, "tools.1.name").String())
	require.Equal(t, "team__send", gjson.GetBytes(adapted, "tools.2.name").String())
}
...
func TestAntigravityCompatResponsesRestoresCustomTools(t *testing.T) {
	gin.SetMode(gin.TestMode)
	const upstreamBody = `data: {"response":{"responseId":"resp_3757","candidates":[{"content":{"parts":[{"functionCall":{"id":"call_3757","name":"get_weather","args":{"city":"Tokyo"}}}]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":8,"candidatesTokenCount":3}}}` + "\n\n"
	mapping := &apicompat.ResponsesClientToolMapping{CustomTools: map[string]bool{"get_weather": true}}
	...
	tests := []struct {
	...
	}{
		{
			name: "non-streaming",
			run: func(svc *AntigravityGatewayService, c *gin.Context, resp *http.Response) (*antigravityStreamResult, error) {
				return svc.handleResponsesNonStreamingFromAntigravityWithClientTools(c, resp, time.Now(), "gemini-3.1-pro-high", mapping)
			},
		},
		{
			name: "streaming",
			run: func(svc *AntigravityGatewayService, c *gin.Context, resp *http.Response) (*antigravityStreamResult, error) {
				return svc.handleResponsesStreamingFromAntigravityWithClientTools(c, resp, time.Now(), "gemini-3.1-pro-high", mapping)
			},
		},
	}
