...
func TestGetModelPricing_UnknownGrokTextFallsBackToGrok45(t *testing.T) {
	...
	// Per-unit media ids must not inherit the text card simply because they
	// include a version number. Multimodal chat models remain token billed.
	...
	for _, model := range []string{
		...
		require.ErrorIs(t, err, ErrModelPricingUnavailable, "non-text grok family %s must not inherit grok-4.5 token rates", model)
	}

	build, err := svc.GetModelPricing("grok-build-latest")
	require.NoError(t, err)
