...
func sanitizeOpenAIResponsesInputItemIDs(body []byte) ([]byte, bool, error) {
	...
	input.ForEach(func(_, item gjson.Result) bool {
		...
		if item.IsObject() {
			...
			id := item.Get("id")
			trimmedType := strings.TrimSpace(itemType.String())
			parsed.stripCallID = item.Get("call_id").Exists() && shouldStripOpenAIResponsesNonPairCallID(trimmedType)
			if id.Type == gjson.String {
				parsed.stripID = shouldStripOpenAIResponsesInputItemID(trimmedType, id.String())
			}
		}
