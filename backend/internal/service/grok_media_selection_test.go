...
func TestGrokVideoStickySelectionPreservesOwnerDespiteHealthStats(t *testing.T) {
	groupID := int64(24)
	...
	require.NoError(t, svc.BindGrokMediaVideoRequestAccount(context.Background(), &groupID, "task", 10, 20, 1))
	selection, decision, err := svc.SelectGrokMediaVideoRequestAccount(
		context.Background(), &groupID, GrokMediaVideoRequestSessionHash("task", 10, 20), 1, "",
	)
	require.NoError(t, err)
	require.True(t, selection.Acquired)
	require.True(t, decision.StickySessionHit)
