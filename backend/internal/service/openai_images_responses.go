...
func (s *OpenAIGatewayService) handleOpenAIImagesErrorResponse(
	...
	// A retired or temporarily unavailable Responses driver is distinct from
	// the image_generation tool model. Surface it directly instead of cooling
	// every image-capable OAuth account.
	...
	shouldFailover := shouldDisable || (isOpenAIOAuthAccount(account) && resp.StatusCode == http.StatusTooManyRequests && failoverErr.RetryableOnSameAccount)
	kind := "http_error"
	if shouldFailover {
		kind = "failover"
	}
