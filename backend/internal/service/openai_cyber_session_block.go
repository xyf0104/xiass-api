...
// scope-gated transcript matching. All storage failures remain fail-open.
func (s *OpenAIGatewayService) FindCyberSessionBlockedForRequest(ctx context.Context, apiKeyID int64, c *gin.Context, body []byte, clientIP, userAgent string) string {
	...
	if transcript.lookupKeysTruncated {
		// Once a source scope is active, silently dropping old candidates would
		// allow bypassing prefix matching by appending dummy transcript items.
