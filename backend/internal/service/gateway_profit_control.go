...
func profitControlVetoLatest(ctx context.Context, selected *Account, snapshot *SchedulerSnapshotService) (*Account, bool, string) {
	...
	if snapshot != nil {
		...
		if err != nil || refreshed == nil {
			...
			if selected.RequestProxy != nil {
				cloned := *refreshed
				cloned.RequestProxy = selected.RequestProxy
				cloned.RequestProxyMaxConcurrency = selected.RequestProxyMaxConcurrency
				cloned.Concurrency = selected.Concurrency
				latest = &cloned
			} else {
				latest = refreshed
			}
