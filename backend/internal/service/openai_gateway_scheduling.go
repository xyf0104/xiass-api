...
// inherit a stale account binding.
...
func isOpenAICompatibleAccountEligibleForRequest(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) bool {
	return isOpenAICompatibleAccountEligibleForRequestWithSettings(ctx, account, platform, requestedModel, requireCompact, requiredCapability, nil)
}

func isOpenAICompatibleAccountEligibleForRequestWithSettings(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability, settings *SettingService) bool {
	...
	if requestedModel != "" && !grokModelSupportedBySettings(ctx, account, requestedModel, settings) {
		return false
	}
...
func grokModelSupportedBySettings(ctx context.Context, account *Account, requestedModel string, settings *SettingService) bool {
	if account == nil {
		return false
	}
	...
	if !account.IsGrok() || len(account.GetModelMapping()) != 0 || settings == nil {
		return false
	}
	return settings.GetGrokCrossClientModelMapEnabled(ctx) && isGrokCrossClientModel(requestedModel)
}
...
// Absolute timestamps win. Relative reset durations must be anchored to the
// observation timestamp so repeated scheduler reads cannot move the window.
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, error) {
	...
	// 1. 尝试粘性会话命中。绑定是否仍在一分钟窗口内由缓存 TTL 决定；
	// 命中后继续使用原账号，不受新会话优先级排序影响。
	...
	selected, compactBlocked := s.selectBestAccount(ctx, groupID, platform, accounts, sessionHash, requestedModel, excludedIDs, requireCompact, requiredCapability, preferLowUpstreamRate)
	if selected == nil {
		...
		if recovered, ok := s.recoverSchedulableAccountsFromDatabase(ctx, groupID, platform, "legacy_selection_exhausted"); ok {
			selected, compactBlocked = s.selectBestAccount(ctx, groupID, platform, recovered, sessionHash, requestedModel, excludedIDs, requireCompact, requiredCapability, preferLowUpstreamRate)
		}
	}
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, platform string, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if !isOpenAICompatibleAccountEligibleForRequestWithSettings(ctx, account, platform, requestedModel, false, requiredCapability, s.settingService) {
		return nil
	}
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool) {
	platform = normalizeOpenAICompatiblePlatform(platform)
	...
	eligible = filterExecutionNodeCandidates(eligible, func(account *Account) *Account { return account }, executionNodePolicy)
	if len(eligible) == 0 {
		return nil, compactBlocked
	}
	...
	eligible = orderExecutionNodeCandidates(
		eligible,
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	...
	if sessionHash != "" && stickyAccountID > 0 && !isExcluded(stickyAccountID) {
		...
		if err == nil {
			...
			eligible := !clearSticky && isOpenAICompatibleAccountEligibleForRequestWithSettings(ctx, account, platform, requestedModel, false, requiredCapability, s.settingService)
			if eligible {
				account = s.recheckSelectedOpenAIAccountFromDB(ctx, account, groupID, platform, requestedModel, requireCompact, requiredCapability)
			}
			if account == nil || clearSticky || !eligible || !s.openAIAccountMatchesSchedulingGroup(account, groupID) ||
				...
				result, acquireErr := s.tryAcquireAccountSlot(ctx, stickyAccountID, account.Concurrency)
				if acquireErr == nil && result != nil && result.Acquired {
	...
	// ============ Layer 2: Build eligible non-sticky candidates ============
	...
	for i := range accounts {
		...
		if !isOpenAICompatibleAccountEligibleForRequestWithSettings(ctx, acc, platform, requestedModel, false, requiredCapability, s.settingService) {
			continue
		}
		...
		if !executionNodeCandidateAllowed(executionNodePolicy, acc) {
			continue
		}
		baseCandidateCount++
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccount(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if !isOpenAICompatibleAccountEligibleForRequestWithSettings(ctx, fresh, platform, requestedModel, requireCompact, requiredCapability, s.settingService) {
		return nil
	}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDB(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if s.schedulerSnapshot == nil || s.accountRepo == nil {
		if !executionNodePolicy.hydratedAccountEgressAllowed(account) {
			return nil
		}
	...
	if !executionNodePolicy.hydratedAccountEgressAllowed(latest) {
		return nil
	}
