...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	if account.IsOpenAIOAuth() {
		aliased, reverse, _, aliasErr := aliasOpenAIOAuthReservedToolNamesBody(firstClientMessage)
		if aliasErr != nil {
			return NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, aliasErr.Error(), aliasErr)
		}
		firstClientMessage = aliased
	...
	routingFields := gjson.GetManyBytes(firstClientMessage, "model", "service_tier")
	...
	var turnHasSemanticOutput atomic.Bool
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if !isResponseCreate && hooks != nil && hooks.BeforeFrame != nil {
				if err := hooks.BeforeFrame(turnNo, payload, strings.TrimSpace(gjson.GetBytes(payload, "session.model").String())); err != nil {
					return payload, nil, err
				}
			}
			...
			if policyErr == nil && blocked == nil && account.IsOpenAIOAuth() {
				aliased, reverse, _, aliasErr := aliasOpenAIOAuthReservedToolNamesBody(out)
				if aliasErr != nil {
					return nil, nil, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, aliasErr.Error(), aliasErr)
				}
				out = aliased
				updateCodexToolNameReverseForWSFrame(c, out, reverse)
			}
			...
			if policyErr == nil && blocked == nil && isResponseCreate {
				...
				turnHasSemanticOutput.Store(false)
				acceptedTurn = true
			}
	...
	relayResult, relayExit := openaiwsv2.RunEntry(openaiwsv2.EntryInput{
		...
		Options: openaiwsv2.RelayOptions{
			...
			AfterClientWrite: func(msgType coderws.MessageType, payload []byte, writeErr error) {
				if msgType == coderws.MessageText && writeErr == nil {
					...
					if openAIWSPassthroughStartsSemanticOutput(payload) {
						turnHasSemanticOutput.Store(true)
					}
				}
				if msgType == coderws.MessageText && openAIWSPassthroughIsTerminalOutput(payload) {
			...
			BeforeRelayCancel: func(exit openaiwsv2.RelayExit) {
				...
				// Keep the reason within the WebSocket close-control-frame limit;
				// otherwise clients may observe EOF instead of the intended status.
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				isPreOutputRateLimit := eventType == "error" && !turnHasSemanticOutput.Load() && isOpenAIWSRateLimitError(errCodeRaw, errTypeRaw, errMsgRaw)
				...
				if turnHasSemanticOutput.Load() || !isOpenAIWSRateLimitError(errCodeRaw, errTypeRaw, errMsgRaw) {
					return nil
				}
