...
const (
	...
	openAIWSExecutionScopeContextKey = "openai_ws_execution_scope"
)

type openAIWSExecutionScopeSnapshot struct {
	scope    string
	threadID string
}

func captureOpenAIWSExecutionScope(c *gin.Context, body []byte, apiKeyID int64) (string, string) {
	if c != nil {
		if value, ok := c.Get(openAIWSExecutionScopeContextKey); ok {
			if snapshot, ok := value.(openAIWSExecutionScopeSnapshot); ok {
				return snapshot.scope, snapshot.threadID
			}
		}
	}
	scope, threadID := resolveOpenAIWSExecutionScope(c, body, apiKeyID)
	if c != nil {
		c.Set(openAIWSExecutionScopeContextKey, openAIWSExecutionScopeSnapshot{scope: scope, threadID: threadID})
	}
