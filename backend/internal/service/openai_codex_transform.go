...
func ensureOpenAIResponsesImageGenerationTool(reqBody map[string]any) bool {
	...
	// Codex may already expose image generation as a function tool. Injecting
	// the hosted tool alongside it creates a conflicting duplicate capability.
	if hasCodexImageGenerationFunctionTool(reqBody) {
		return false
	}
	// Codex App can provide its own image_gen namespace tool. Adding the hosted
	// image_generation tool beside it makes OpenAI reject the request as a conflict.
