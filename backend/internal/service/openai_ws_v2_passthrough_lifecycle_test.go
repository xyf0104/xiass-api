...
func TestPassthroughLifecycle_CyberTerminalEventsMarkBeforeAfterTurn(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			message, err := readPassthroughLifecycleFrame(t, clientConn, 3*time.Second)
			require.NoError(t, err)
			...
			case relayErr := <-serverErr:
				require.ErrorIs(t, relayErr, ErrOpenAIWSCyberPolicyBlocked)
			case <-time.After(3 * time.Second):
...
func TestPassthroughLifecycle_CyberSkipsFailureAccountSideEffects(t *testing.T) {
	...
	require.Equal(t, OpenAIWSCyberPolicyClientMessage, gjson.GetBytes(event, "error.message").String())
