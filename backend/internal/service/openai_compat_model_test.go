...
func TestOpenAICompatPreviousResponseUnavailableRecognitionIsStrict(t *testing.T) {
	...
	require.True(t, isOpenAICompatPreviousResponseUnsupported(http.StatusBadRequest, "", body))
...
func TestForwardAsAnthropic_ReusesOAuthCodexTurnState(t *testing.T) {
	...
	require.Equal(t, generateSessionUUID(isolateOpenAIUpstreamSessionID(0, codexAccountIdentitySource(secondCtx, account), "stable-cache-key")), upstream.requests[1].Header.Get("session_id"))
