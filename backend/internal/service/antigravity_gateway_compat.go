...
type antigravityCompatRequest struct {
	...
	clientToolMap   *apicompat.ResponsesClientToolMapping
}
...
func (s *AntigravityGatewayService) ForwardAsResponses(
	...
	request, _, clientToolMap, err := adaptAntigravityResponsesRequest(body)
	if err != nil {
		return nil, s.writeAntigravityCompatError(c, http.StatusBadRequest, "invalid_request_error", "Failed to parse request body")
	}
	if strings.TrimSpace(request.Model) == "" {
	...
	return s.forwardAntigravityCompat(ctx, c, account, antigravityCompatRequest{
		...
		clientToolMap:   clientToolMap,
	})
}

func adaptAntigravityResponsesRequest(body []byte) (*apicompat.ResponsesRequest, []byte, *apicompat.ResponsesClientToolMapping, error) {
	...
	mapping, changed, err := apicompat.AdaptResponsesClientTools(raw)
	if err != nil {
		return nil, nil, nil, err
	}
	if !changed {
	...
	adaptedBody, err := json.Marshal(raw)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	return &request, adaptedBody, &mapping, nil
}
...
func (s *AntigravityGatewayService) consumeAntigravityCompatSuccess(
	...
	if call.request.clientStream {
		...
		return s.handleResponsesStreamingFromAntigravityWithClientTools(
			c,
			resp,
			call.request.startTime,
			call.request.originalModel,
			call.request.clientToolMap,
		)
	}
...
func (s *AntigravityGatewayService) handleResponsesNonStreamingFromAntigravity(
	...
	return s.handleResponsesNonStreamingFromAntigravityWithClientTools(c, resp, startTime, originalModel, nil)
}

func (s *AntigravityGatewayService) handleResponsesNonStreamingFromAntigravityWithClientTools(
	c *gin.Context,
	resp *http.Response,
	startTime time.Time,
	originalModel string,
	...
	responsesResponse := apicompat.AnthropicToResponsesResponse(&anthropicResponse)
	if clientToolMap == nil {
		c.JSON(http.StatusOK, responsesResponse)
		return result, nil
	}
	payload, err := json.Marshal(responsesResponse)
	if err != nil {
		return nil, s.writeAntigravityCompatError(c, http.StatusBadGateway, "upstream_error", "Failed to encode upstream response")
	}
	restored, _, err := apicompat.RestoreResponsesClientToolPayload(payload, *clientToolMap)
	if err != nil {
