...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		ModelPricingEnabled:      !isFalseSettingValue(settings[SettingKeyModelPricingEnabled]),
...
// NormalizeTencentCaptchaRegion accepts only the international Tencent site as
// an override. Empty and invalid stored values retain the mainland default so
// older installations remain compatible.
func NormalizeTencentCaptchaRegion(raw string) string {
	if strings.EqualFold(strings.TrimSpace(raw), TencentCaptchaRegionINTL) {
...
// the settings store. Fail-open keeps the existing active-probe implementation
// available if the setting repository is temporarily unavailable.
