...
// scheduling contract. DeepSeek is balance-based and is intentionally absent.
...
const (
	...
	SettingKeyTencentCaptchaRegion         = "tencent_captcha_region" // 站点："cn" | "intl"
	...
	// SettingKeyChannelMonitorHideThroughput hides user-facing RPM/TPM style
	// throughput values. Admin monitor APIs always retain the complete metrics.
	SettingKeyChannelMonitorHideThroughput = "channel_monitor_hide_throughput"

	// SettingKeyChannelMonitorShowQuota controls user-facing quota/balance
	// snapshots. It is fail-closed: only the literal "true" exposes them.
	SettingKeyChannelMonitorShowQuota = "channel_monitor_show_quota"

	// Grok default mapping policy. These values are settings data, while the
	// gateway owns their runtime interpretation.
	...
	// SettingKeyModelPricingEnabled independently controls XIASS's model-price
	// presentation. It defaults to true, unlike the opt-in available-channels view.
	SettingKeyModelPricingEnabled = "model_pricing_enabled"

	// Model Plaza remains an independent upstream public showcase feature.
	...
	// SettingKeyExecutionNodeBalancingEnabled enables weighted selection between
	// account execution nodes. The local gateway.execution_node switch remains
	// the deployment-level safety gate.
