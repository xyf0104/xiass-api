...
func (s *OpenAIGatewayService) handleOpenAIAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, canonicalModel ...string) bool {
	...
	// Team linked fuse must run before model-specific and temporary blocking
	// rules return early. The method deduplicates the second RateLimit entry.
...
func (s *OpenAIGatewayService) ShouldStopOpenAIOAuth429Failover(account *Account, statusCode int, failedSwitches int, state *OpenAIOAuth429FailoverState) bool {
	...
	// Each OpenAI OAuth candidate has already consumed its bounded same-account
	// retry window before reaching this switch point. Keep the cross-account
	// budget deterministic; a process-wide 429 counter must not make the result
	// depend on unrelated concurrent requests.
