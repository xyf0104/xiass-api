...
//
// The normal form is normalizeMonitorPrimaryModel(provider, checkMode, model).
// The optional final argument temporarily preserves the historic private
// two-argument probe-mode caller while this rolling merge is in progress.
func normalizeMonitorPrimaryModel(provider, checkMode string, model ...string) string {
	primaryModel := checkMode
	if len(model) > 0 {
	...
	if primaryModel == "" && defaultCheckMode(checkMode) == MonitorCheckModeQuota {
		return MonitorDefaultQuotaModel
	}
	if primaryModel == "" && provider == MonitorProviderGrok {
