...
func TestUpdateAccountPreservesCodexTicketOnEdit(t *testing.T) {
	...
	account.Extra = map[string]any{key: ticket, OpenAICodexTicketEnabledExtraKey: true}
	repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{41: account}}
	svc := &adminServiceImpl{accountRepo: repo}
	for _, extra := range []map[string]any{{key: map[string]any{"model": ticket.Model, "length": 292}, OpenAICodexTicketEnabledExtraKey: false, "custom": true}, {"custom": true}} {
...
func TestCreateAccountDropsUserSuppliedCodexTickets(t *testing.T) {
	account, err := buildAccountForCreate(&CreateAccountInput{Platform: PlatformOpenAI, Type: AccountTypeOAuth}, map[string]any{
		"codex_turn_ticket:custom":       map[string]any{"state": "spoofed"},
