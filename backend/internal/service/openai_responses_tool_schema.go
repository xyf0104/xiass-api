...
// 账号池里反复重放同一份坏 Schema。
//
// 缺失 type 通常保持原样；唯一例外是参数根节点的 oneOf/anyOf 每个分支都明确
// 为 object，此时补写 object 不会收窄客户端语义，并可兼容上游严格校验。
//
...
func collectOpenAIResponsesToolSchemaNullTypes(
	body []byte, tools gjson.Result, depth int, hits *[]openAIResponsesToolSchemaEdit,
) {
	...
	tools.ForEach(func(_, tool gjson.Result) bool {
		...
		for _, suffix := range []string{"parameters", "function.parameters"} {
			...
			if typ.Type == gjson.Null && typ.Raw == openAIResponsesToolSchemaNullLiteral {
				...
				appendOpenAIResponsesMissingObjectUnionType(body, params, hits)
			}
		}
...
func appendOpenAIResponsesMissingObjectUnionType(
	body []byte, params gjson.Result, hits *[]openAIResponsesToolSchemaEdit,
) {
	if params.Index <= 0 || params.Index+len(params.Raw) > len(body) ||
		!bytes.Equal(body[params.Index:params.Index+len(params.Raw)], []byte(params.Raw)) {
		return
	}
	if replacement, ok := openAIMissingRootObjectUnionType(params.Raw); ok {
		*hits = append(*hits, openAIResponsesToolSchemaEdit{
...
func openAIMissingRootObjectUnionType(raw string) (string, bool) {
	if len(raw) > openAIResponsesObjectUnionMaxSize {
		return "", false
	}
	var schema map[string]json.RawMessage
	if len(raw) < 2 || raw[0] != '{' || json.Unmarshal([]byte(raw), &schema) != nil {
		return "", false
	}
