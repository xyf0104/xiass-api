...
type ModelPricing struct {
	...
	LongContextThresholdInclusive      bool    // 达到阈值即应用；默认保持严格大于
...
func (s *BillingService) initFallbackPricing() {
	...
	fableMaxReasoningMultiplier := 3.0
	...
	// Gemini 3.7 Flash temporarily reuses the verified 3.6 rate until Google
	// publishes a distinct card. Channel pricing still takes precedence.
	...
	// Gemini 3.8 Flash promotional rates through 2026-12-31. Its public
	// reasoning tiers and internal tiered route share this price card.
	...
	// GPT-6 Astra keeps the XIASS price card already used by the admin UI and
	// production billing. This fallback only applies when dynamic pricing is
	// unavailable; channel overrides and group multipliers remain authoritative.
	...
	// GLM-5.3-Flash 使用列表价 $0.15/$0.50 per MTok。
	...
	// input, cached input, and output are all 2x from 200k onward.
...
func (s *BillingService) getFallbackPricing(model string) *ModelPricing {
	...
	// 注意：glm-5.3-flash 必须先于 glm-5.3，带小数点型号必须先于裸 glm-5。
	...
	// Unknown Grok text IDs inherit the current default text card so a new
	// text model cannot silently become unbilled. Media families are billed by
	// image/video/audio units and must never enter this token-pricing fallback.
...
// isGrokMediaFamilyModel matches IDs priced per image/video/audio unit rather
// than per token, so they cannot pick up a text fallback by accident.
...
// HasIdentifiedTokenPricing reports whether a token price is backed by a
// deterministic catalog or exact fallback entry, rather than a family guess.
...
// an explicit model-catalog Fast/Priority ratio.
...
type CostInput struct {
	...
	// PricingAt freezes time-dependent pricing for historical accounting.
	PricingAt                 time.Time
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	// A group may deliberately disable long-context tiers. An account-level
	// opt-in remains an additive override for the existing compatibility path.
	...
	if !contextTierPricingEnabled {
		// Select the lowest interval rather than an arbitrarily high tier when
		// long-context pricing is disabled. Flat channel pricing still wins.
		pricingContext = 1
	}
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool, pricingAt time.Time) *ModelPricing {
	...
	needsCacheCreationPolicy := !pricing.CacheCreationPriceExplicit && (pricing.CacheCreationPricePerToken <= 0 ||
	...
	if !isGPT56 {
		needsCacheCreationPolicy = false
		needsLongContextPolicy = false
	}
	if !needsCacheCreationPolicy && !needsLongContextPolicy && !needsMaxReasoningEffortMultiplier {
		return pricing
	}
	cloned := *pricing
