...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_UnknownSizesDoNotBlockPassthrough(t *testing.T) {
	...
	tests := []struct {
		...
		{size: "invalid", wantTier: "1K"},
		{size: "999999999999999999999999999x2", wantTier: "1K"},
	}
...
func TestOpenAIGatewayServiceForwardImages_APIKeyGenerationUsesConfiguredV1BaseURL(t *testing.T) {
	...
	require.Equal(t, xiassDefaultOpenAIImageSize, gjson.GetBytes(upstream.lastBody, "size").String())
	require.Equal(t, http.StatusOK, rec.Code)
...
func TestOpenAIGatewayServiceForwardImages_APIKeyTransportErrorReturnsFailover(t *testing.T) {
	...
	account := &Account{
		ID:       61,
		Name:     "openai-apikey-network-failure",
		Platform: PlatformOpenAI,
		Type:     AccountTypeAPIKey,
		Credentials: map[string]any{
			"api_key":  "test-api-key",
...
func TestOpenAIGatewayServiceForwardImages_OAuthTransportErrorReturnsFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","response_format":"b64_json"}`)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	...
	account := &Account{
		ID:       62,
		Name:     "openai-oauth-network-failure",
		Platform: PlatformOpenAI,
		Type:     AccountTypeOAuth,
		Credentials: map[string]any{
...
func TestOpenAIGatewayServiceForwardImages_APIKeyEditUsesConfiguredV1BaseURL(t *testing.T) {
	...
	require.Contains(t, string(upstream.lastBody), "3840x2160")
	require.Equal(t, http.StatusOK, rec.Code)
