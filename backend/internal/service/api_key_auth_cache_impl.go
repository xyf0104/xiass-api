...
const apiKeyAuthSnapshotVersion = 22 // v22: include complete group Codex, media, and routing policy
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	if apiKey.Group != nil {
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			ModelRoutingPools:               apiKey.Group.ModelRoutingPools,
			...
			CostRatio:                       apiKey.Group.CostRatio,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			ModelRoutingPools:               snapshot.Group.ModelRoutingPools,
			...
			CostRatio:                       snapshot.Group.CostRatio,
