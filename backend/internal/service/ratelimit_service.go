...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	ctx = withTempUnschedulableModel(ctx, requestedModel)
	// Team-linked OpenAI failures must be observed before any pool/custom-policy
	// early return. The helper deduplicates repeated observations.
	...
	case 402:
		// CN provider balance exhaustion is recoverable after recharge or the next
		// successful quota probe, so it must not permanently mark the account error.
...
func (s *RateLimitService) persistOpenAICodexSnapshot(ctx context.Context, account *Account, headers http.Header) {
	...
	if _, err := persistOrderedOpenAICodexSnapshot(ctx, s.accountRepo, account, updates); err != nil {
...
func (s *RateLimitService) HandleOpenAICodexSparkRateLimit(ctx context.Context, account *Account, requestedModel string, statusCode int, headers http.Header, responseBody []byte) bool {
	...
	modelKey := normalizeCodexModel(modelRateLimitKeyForUpstreamFailure(ctx, account, requestedModel))
	if modelKey == "" {
		return false
	}
	now := time.Now()
	disposition, resetAt := classifyOpenAIOAuth429(headers, responseBody)
	// Only a confirmed exhausted Spark 5h/7d window may use an upstream long
	// reset. A transient 429 with incidental quota headers gets a short retry.
