...
func TestTryModelFilePricing_Fable51MaxEffortUsesTripleQuota(t *testing.T) {
	...
	maxCost := tryModelFilePricing(bs, "claude-fable-5-1", tokens, "", "max")
	require.NotNil(t, standard)
	require.NotNil(t, maxCost)
	require.InDelta(t, *standard*3, *maxCost, 1e-12)
}
...
func TestTryModelFilePricing_CombinesPriorityAndLongContextPricing(t *testing.T) {
	...
	require.NotNil(t, result)
	// Priority prices are applied before long-context input/output multipliers.
...
func TestResolveAccountStatsCost_HitsCustomRule(t *testing.T) {
	...
	result := resolveAccountStatsCost(
		...
		tokens, 1, 999.0, "priority", // Custom account prices do not apply service-tier pricing.
	)
	require.NotNil(t, result)
...
func TestResolveAccountStatsCost_ApplyPricingToAccountStats_UsesTotalCost(t *testing.T) {
	...
	result := resolveAccountStatsCost(
		...
		cs, nil,
		1, 10, "claude-sonnet-4",
		tokens, 1, 0.75, "priority", // Already-billed totalCost must not apply service-tier pricing twice.
	)
	require.NotNil(t, result)
...
func TestResolveAccountStatsCost_Gemini37PublicTierUsesFallbackPricing(t *testing.T) {
	channel := &Channel{
		ID:                         1,
		Status:                     StatusActive,
		ApplyPricingToAccountStats: false,
...
func TestApplyAccountStatsCost_Gemini37TieredUsesPublicTierCustomPricing(t *testing.T) {
	channel := &Channel{
		ID:                         1,
		Status:                     StatusActive,
		ApplyPricingToAccountStats: false,
				...
				Pricing: []ChannelModelPricing{
					{
						...
						Models:         []string{"gemini-3.7-flash-high"},
						BillingMode:    BillingModeToken,
	...
	applyAccountStatsCost(
		...
		1, 10, "gemini-3.7-flash-tiered", "gemini-3.7-flash-high",
