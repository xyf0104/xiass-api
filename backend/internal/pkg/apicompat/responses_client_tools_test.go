...
func TestAdaptResponsesClientTools_PreservesFunctionProtocolItemID(t *testing.T) {
	req := map[string]any{
		"tools": []any{map[string]any{"type": "custom", "name": "exec"}},
		"input": []any{map[string]any{
			"type": "custom_tool_call", "id": "fc_valid", "call_id": "call_1", "name": "exec", "input": "pwd",
		}},
	}
	...
	items := requireResponsesClientToolValue[[]any](t, req["input"])
	call := requireResponsesClientToolValue[map[string]any](t, items[0])
	require.Equal(t, "fc_valid", call["id"])
}
