...
// Old installations may retain this marker; neither presence nor absence permits deleting peer waiters.
const legacyWaitSweepMarkerKey = "concurrency:startup:legacy_wait_sweep:v1"
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots() {
	// Existing legacy markers must not affect expiry-based cleanup.
	...
	userWaiting, err := s.rdb.Get(s.ctx, userWaitKey).Int()
	require.NoError(s.T(), err)
	require.Equal(s.T(), 3, userWaiting)

	accountWaiting, err := s.rdb.Get(s.ctx, accountWaitKey).Int()
	require.NoError(s.T(), err)
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_PreservesUnindexedWaitCounters() {
	...
	// First startup must preserve unindexed peer waiters even without the legacy marker.
	require.NoError(s.T(), s.cache.CleanupStaleProcessSlots(s.ctx, "keep-"))

	accountWaiting, err := s.rdb.Get(s.ctx, unindexedAccountWaitKey).Int()
	require.NoError(s.T(), err)
	require.Equal(s.T(), 5, accountWaiting)
	userWaiting, err := s.rdb.Get(s.ctx, unindexedUserWaitKey).Int()
	require.NoError(s.T(), err)
	require.Equal(s.T(), 3, userWaiting)

	exists, err := s.rdb.Exists(s.ctx, legacyWaitSweepMarkerKey).Result()
	require.NoError(s.T(), err)
	require.EqualValues(s.T(), 0, exists, "startup must no longer run the destructive legacy sweep")

	// Previously released instances may have already written the marker.
	require.NoError(s.T(), s.rdb.Set(s.ctx, legacyWaitSweepMarkerKey, "1", 0).Err())
	require.NoError(s.T(), s.rdb.Set(s.ctx, unindexedAccountWaitKey, 5, time.Minute).Err())
	require.NoError(s.T(), s.cache.CleanupStaleProcessSlots(s.ctx, "keep-"))
	val, err := s.rdb.Get(s.ctx, unindexedAccountWaitKey).Int()
	require.NoError(s.T(), err, "existing migration markers must not change waiter preservation")
	require.Equal(s.T(), 5, val)
}

func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_ProcessesExpiredIndexMembers() {
	// Expired index scores are candidates, not proof that waiters have expired.
	require.NoError(s.T(), s.rdb.Set(s.ctx, legacyWaitSweepMarkerKey, "1", 0).Err())
	...
	waiting, err := s.rdb.Get(s.ctx, accountWaitKey).Int()
	require.NoError(s.T(), err)
	require.Equal(s.T(), 4, waiting, "waiters must survive even when every slot expired")

	score, err := s.rdb.ZScore(s.ctx, accountActiveIndexKey, strconv.FormatInt(accountID, 10)).Result()
	require.NoError(s.T(), err)
	require.Greater(s.T(), int64(score), now, "wait-only load must retain its active index")
	_, err = s.rdb.ZScore(s.ctx, userActiveIndexKey, strconv.FormatInt(userID, 10)).Result()
	require.ErrorIs(s.T(), err, redis.Nil)
}

func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_PreservesOtherPrefixesAndWaitCounters() {
	// A live peer must survive even when the current process already owns slots.
	require.NoError(s.T(), s.rdb.Set(s.ctx, legacyWaitSweepMarkerKey, "1", 0).Err())
	accountID := int64(901)
	userID := int64(902)
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_DeletesEmptySlotKeys() {
	...
	now, err := s.rawCache.redisUnixSeconds(s.ctx)
	require.NoError(s.T(), err)
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountSlotKey, redis.Z{Score: float64(now - int64(testSlotTTL.Seconds())), Member: "oldproc-1"}).Err())
	require.NoError(s.T(), s.rdb.Expire(s.ctx, accountSlotKey, testSlotTTL).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountActiveIndexKey, redis.Z{
		Score:  float64(now + 60),
		Member: strconv.FormatInt(accountID, 10),
