...
func (r *settingRepository) ReadExecutionNodeFence(ctx context.Context) (service.ExecutionNodeFenceRecord, error) {
	if r == nil || r.db == nil {
		return service.ExecutionNodeFenceRecord{}, errors.New("settings database is unavailable")
	}
	var raw string
	if err := r.db.QueryRowContext(ctx, `SELECT value FROM settings WHERE key = $1`, service.SettingKeyExecutionNodeFence).Scan(&raw); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return service.ExecutionNodeFenceRecord{}, service.ErrSettingNotFound
		}
		return service.ExecutionNodeFenceRecord{}, err
	}
	var record service.ExecutionNodeFenceRecord
	if err := json.Unmarshal([]byte(raw), &record); err != nil {
		return service.ExecutionNodeFenceRecord{}, fmt.Errorf("decode execution-node fence: %w", err)
	}
	return record, nil
}

// CommitExecutionNodeFence is the PostgreSQL half of the witness fence. It is
// monotonic and holder-stable within one generation, so a stale process cannot
// lower or repurpose a generation even if it still has database connectivity.
func (r *settingRepository) CommitExecutionNodeFence(ctx context.Context, record service.ExecutionNodeFenceRecord) (bool, error) {
	if r == nil || r.db == nil {
		return false, errors.New("settings database is unavailable")
	}
	if strings.TrimSpace(record.ClusterID) == "" || strings.TrimSpace(record.HolderNodeID) == "" ||
		strings.TrimSpace(record.LeaseID) == "" || record.Generation <= 0 || record.ExpiresAt.IsZero() {
		return false, errors.New("execution-node fence record is incomplete")
	}
	...
	err = tx.QueryRowContext(ctx, `SELECT value FROM settings WHERE key = $1 FOR UPDATE`, service.SettingKeyExecutionNodeFence).Scan(&currentRaw)
	switch {
	case errors.Is(err, sql.ErrNoRows):
		if _, err = tx.ExecContext(ctx, `INSERT INTO settings (key, value, updated_at) VALUES ($1, $2, NOW())`, service.SettingKeyExecutionNodeFence, string(encoded)); err != nil {
			return false, err
		}
	case err != nil:
		return false, err
	default:
		var current service.ExecutionNodeFenceRecord
		if err := json.Unmarshal([]byte(currentRaw), &current); err != nil {
			return false, fmt.Errorf("decode current execution-node fence: %w", err)
		}
		if current.ClusterID != record.ClusterID || current.Generation > record.Generation ||
			(current.Generation == record.Generation && (current.HolderNodeID != record.HolderNodeID || current.LeaseID != record.LeaseID)) {
			return false, nil
		}
		if _, err = tx.ExecContext(ctx, `UPDATE settings SET value = $1, updated_at = NOW() WHERE key = $2`, string(encoded), service.SettingKeyExecutionNodeFence); err != nil {
			return false, err
		}
	}
	if err := tx.Commit(); err != nil {
