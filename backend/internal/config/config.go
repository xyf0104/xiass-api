...
func configureConfigSource(setConfigFile, addConfigPath func(string)) {
	...
	addConfigPath("/etc/xiass-api")
	addConfigPath("/etc/nowind-api")
...
func setDefaults() {
	...
	viper.SetDefault("log.rotation.max_size_mb", 10)
	viper.SetDefault("log.rotation.max_backups", 3)
	...
	viper.SetDefault("webauthn.rp_display_name", "XIASS API")
	...
	viper.SetDefault("gateway.openai_ws.sticky_session_ttl_seconds", 60)
...
func (c *Config) Validate() error {
	...
	switch c.JWT.RefreshTokenStore {
	case "", "redis", "postgres":
	default:
