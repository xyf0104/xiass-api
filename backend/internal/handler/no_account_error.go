...
// every model-capable account is temporarily rate limited. A model-not-found
// result remains authoritative because retrying cannot repair configuration.
func classifySelectionFailureError(err error, fallback noAccountErrorClassification) noAccountErrorClassification {
	if err == nil || fallback.ModelNotFound {
		return fallback
	}
