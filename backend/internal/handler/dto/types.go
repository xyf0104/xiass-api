...
type AdminUser struct {
	...
	// RestrictPublicGroups is an admin-only permission boundary.
	RestrictPublicGroups bool `json:"restrict_public_groups"`
}
...
type Group struct {
	...
	// LongContextPricingEnabled controls whether the existing static long-context
	// model tiers apply. ModelPricing itself remains admin-only below.
	...
	// MaxReasoningEffortOverLimit controls whether over-limit requests are downgraded or denied.
...
type AdminGroup struct {
	...
	// ModelPricing is administration-only. It overrides channel and built-in
...
type Account struct {
	...
	CodexTicketCaptureProxyIDs []int64                           `json:"codex_ticket_capture_proxy_ids"`
...
// AccountListItem is the compact account-list projection. It intentionally
// omits repeated group object graphs while retaining XIASS runtime indicators.
