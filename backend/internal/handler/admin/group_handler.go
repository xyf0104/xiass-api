...
type GroupHandler struct {
	...
	allowlistService     *service.AdminUserGroupAccountAllowlistService
	cfg                  *config.Config
}
...
// SetUserGroupAccountAllowlistService attaches the narrow admin-only
// allowlist bridge after the regular handler graph is built.
func (h *GroupHandler) SetUserGroupAccountAllowlistService(svc *service.AdminUserGroupAccountAllowlistService) {
	h.allowlistService = svc
}
...
type CreateGroupRequest struct {
	...
	// nil keeps the XIASS default (enabled); false explicitly disables long-context tiers.
...
func (h *GroupHandler) List(c *gin.Context) {
	...
	if h.isSimpleMode() {
		...
		response.Paginated(c, simpleGroups, int64(len(simpleGroups)), page, pageSize)
		return
	}
...
// GetGroupModelAllowlistCandidates returns candidate model IDs for the
// current group model allowlist. The legacy models-list endpoint remains as a
// compatibility alias for existing XIASS clients.
...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	group, err := h.adminService.CreateGroup(c.Request.Context(), &service.CreateGroupInput{
		...
		ModelRoutingPools:               req.ModelRoutingPools,
