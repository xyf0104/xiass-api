...
func (h *AccountHandler) enrichCodexTicketStatus(account *service.Account, out *dto.Account) {
	...
	out.CodexTicketEnabled = service.OpenAICodexTicketEnabledForAccount(account)
...
func (h *AccountHandler) buildAccountResponseWithRuntime(ctx context.Context, account *service.Account) AccountWithConcurrency {
	...
	if h.concurrencyService != nil {
		...
		h.enrichAccountProxyConcurrency(ctx, account, item.Account)
	}
...
func (h *AccountHandler) enrichAccountProxyConcurrency(ctx context.Context, account *service.Account, out *dto.Account) {
	if h == nil || h.concurrencyService == nil || account == nil || out == nil || len(out.ProxyBindings) == 0 {
		return
	}
	counts, err := h.concurrencyService.GetAccountProxyConcurrency(ctx, account)
	if err != nil {
		return
	}
	applyAccountProxyConcurrency(out, counts)
}

func applyAccountProxyConcurrency(out *dto.Account, counts map[int64]int) {
	if out == nil || len(out.ProxyBindings) == 0 {
		return
	}
	for i := range out.ProxyBindings {
		out.ProxyBindings[i].CurrentConcurrency = counts[out.ProxyBindings[i].ProxyID]
	}
}
...
func (h *AccountHandler) List(c *gin.Context) {
	...
	proxyConcurrencyCounts := make(map[int64]map[int64]int)
	if h.concurrencyService != nil {
		if counts, err := h.concurrencyService.GetAccountsProxyConcurrency(c.Request.Context(), accounts); err == nil {
			proxyConcurrencyCounts = counts
		}
	}
	...
	for i := range accounts {
		acc := &accounts[i]
		accountDTO := h.accountResponseFromService(acc)
		applyAccountProxyConcurrency(accountDTO, proxyConcurrencyCounts[acc.ID])
		item := AccountWithConcurrency{
			Account:            accountDTO,
			simpleMode:         h.isSimpleMode(),
...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	result, err := executeAdminIdempotent(c, "admin.accounts.create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		account, execErr := h.adminService.CreateAccount(ctx, &service.CreateAccountInput{
			...
			Extra:                 req.Extra,
			ProxyID:               req.ProxyID,
			ProxyBindings:         req.ProxyBindings,
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	if account.IsOpenAI() {
		...
		if account.IsOpenAIPassthroughEnabled() {
			response.Success(c, openai.DefaultAccountTestModels())
			return
		}
	...
	if err != nil {
		...
		if errors.As(err, &syncErr) {
			...
			case service.UpstreamModelSyncErrorInternal:
				slog.Error("sync_upstream_models_persist_failed", "account_id", accountID, "error", err)
				response.InternalError(c, "Failed to save upstream model capabilities")
			default:
	...
	if err != nil {
		...
		if errors.As(err, &syncErr) {
			...
			case service.UpstreamModelSyncErrorConfiguration, service.UpstreamModelSyncErrorUnsupported:
				response.BadRequest(c, syncErr.SafeMessage())
			case service.UpstreamModelSyncErrorInternal:
				slog.Error("sync_upstream_models_preview_internal_failed", "platform", req.Platform, "error", err)
				response.InternalError(c, "Failed to prepare upstream model capabilities")
			default:
