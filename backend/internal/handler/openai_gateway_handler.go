...
func validateOpenAIWSModelAllowlist(group *service.Group, payload []byte, fallbackModel string) error {
	if group == nil || !group.ModelAllowlistEnabled() {
		return nil
	}
	candidates := requestmodel.FromBodyCandidates("", "application/json", payload)
	if len(candidates) == 0 && strings.TrimSpace(fallbackModel) != "" {
		candidates = []string{strings.TrimSpace(fallbackModel)}
	}
...
func shouldReportOpenAIWSProxyAccountFailure(err error) bool {
	return err != nil &&
		...
		!service.IsOpenAIWSCyberPolicyBlocked(err)
}
...
// openAIChannelForwardModel returns the model used for account capability
// routing. The client model remains unchanged for request logs and responses.
...
// openAIAccountScheduleModel applies the selected account's model mapping to
// the already channel-mapped routing model. Scheduler state must use this same
// key for selection results and transient-state cleanup; request, billing, log,
// and observed upstream models remain separate concerns.
func openAIAccountScheduleModel(account *service.Account, channelForwardModel string) string {
	model := strings.TrimSpace(channelForwardModel)
...
func openAIWSTurnScheduleModel(account *service.Account, snapshot *openAIWSTurnChannelMappingSnapshot, fallbackRequestedModel string) string {
	requestedModel := strings.TrimSpace(fallbackRequestedModel)
	mapping := service.ChannelMappingResult{}
	if snapshot != nil {
		if snapshotRequestedModel := strings.TrimSpace(snapshot.requestedModel); snapshotRequestedModel != "" {
			requestedModel = snapshotRequestedModel
		}
		mapping = snapshot.mapping
	}
	return openAIAccountScheduleModel(account, openAIChannelForwardModel(mapping, requestedModel))
}
...
const (
	...
	openAIPreviousResponseAffinityErrorCode          = "previous_response_affinity_unavailable"
	openAIPreviousResponseAffinityUnavailableMessage = "previous_response_id affinity unavailable; retry later or start a new response without previous_response_id"
)
...
func openAICompatibleRequestPlatform(ctx context.Context, apiKey *service.APIKey) string {
	if platform, ok := service.ResolvedTargetPlatformFromContext(ctx); ok {
		if platform == service.PlatformGrok || service.IsCNProvider(platform) || service.IsOpenCodeGo(platform) {
			return platform
		}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	nativeCompactionV2 := isBareOpenAIResponsesPath(c) &&
	...
	requiredCapability := openAIResponsesRequiredCapabilityForRequest(imageIntent, requireCompact || nativeCompactionV2, requestPlatform)
	...
	for {
		...
		if err != nil {
			...
			if h.handleOpenAIPreviousResponseAffinityError(c, err, streamStarted) {
				return
			}
		...
		hasBoundSession = hasBoundSession || scheduleDecision.StickyPreviousHit || scheduleDecision.StickySessionHit
		...
		if service.IsOpenAIWSCyberPolicyBlocked(err) {
			return
		}
		...
		// Forward may return a partial result together with an error after the
		// upstream has already consumed tokens. Record that result before any
		// terminal error return. Failover errors are handled separately and must
		// not be billed here because they represent an account switch attempt.
		...
		if err != nil {
			...
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					// Failover admission already proved that any committed bytes are
					// non-semantic keepalives. Exhaustion still needs an SSE error.
					if c.Writer.Written() {
						streamStarted = true
					}
		...
		if result != nil {
			...
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, scheduleModel, openAIForwardSucceededForScheduling(result), nil)
		}
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if failoverErr.ShouldReportAccountScheduleFailure() {
						scheduleModel := openAIAccountScheduleModel(account, currentRoutingModel)
						h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, scheduleModel, false, nil)
						h.gatewayService.ReportOpenAIAccountProxyRouteOutcome(c.Request.Context(), account, scheduleModel, false, true, result)
					}
					if !failoverErr.ShouldRetryNextAccount() {
		...
		if result != nil {
			...
			h.gatewayService.ReportOpenAIAccountProxyRouteOutcome(c.Request.Context(), account, scheduleModel, true, false, result)
		} else {
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, openAIAccountScheduleModel(account, currentRoutingModel), true, nil)
		}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	if err := validateOpenAIWSModelAllowlist(apiKey.Group, firstMessage, reqModel); err != nil {
	...
	handleWSFailover := func(account *service.Account, failoverErr *service.UpstreamFailoverError, scheduleModel string) bool {
		if ctx.Err() != nil {
			return false
		}
		if failoverErr.ShouldReportAccountScheduleFailure() {
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, scheduleModel, false, nil)
			h.gatewayService.ReportOpenAIAccountProxyRouteOutcome(ctx, account, scheduleModel, false, true, nil)
		}
		releaseAccountSlot()
		if !failoverErr.ShouldRetryNextAccount() {
	...
	for {
		...
		if err != nil {
			...
			if closeOpenAIPreviousResponseAffinityError(wsConn, err) {
				return
			}
			if lastFailoverErr != nil {
		...
		if err != nil {
			...
			var failoverErr *service.UpstreamFailoverError
			if errors.As(err, &failoverErr) {
				if handleWSFailover(account, failoverErr, openAIAccountScheduleModel(account, wsForwardModel)) {
					continue
				}
				return
			}
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			BeforeRequest: func(turn int, payload []byte, originalModel string) error {
				...
				// Keep the connection-level cyber session gate effective for both
				// native and passthrough ingress before any upstream write.
				...
				if err := validateOpenAIWSModelAllowlist(apiKey.Group, payload, model); err != nil {
					return service.NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, err.Error(), nil)
				}
			...
			BeforeFrame: func(turn int, payload []byte, originalModel string) error {
				if strings.TrimSpace(gjson.GetBytes(payload, "type").String()) != "session.update" ||
					strings.TrimSpace(gjson.GetBytes(payload, "session.model").String()) == "" {
					return nil
				}
				model := strings.TrimSpace(originalModel)
				if model == "" {
				...
				if decision := h.checkSecurityAuditStage(c, reqLog, apiKey, subject, service.ContentModerationProtocolOpenAIResponses, model, payload, "session_update"); decision != nil && !decision.AllowNextStage {
					writeSecurityAuditWSError(ctx, wsConn, decision)
					return service.NewOpenAIWSClientCloseError(securityAuditWSCloseStatus(decision), securityAuditWSCloseReason(decision), nil)
				}
				return nil
			...
			AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
				...
				// Each failover attempt clears its mark. The recorded guard remains
				// until the logical turn is complete so one turn is logged only once.
				defer func() {
				...
				var turnMappingSnapshot *openAIWSTurnChannelMappingSnapshot
				if snapshot := turnChannelMapping.Load(); snapshot != nil && snapshot.turn == turn {
					turnMappingSnapshot = snapshot
					...
					turnMappingSnapshot = &openAIWSTurnChannelMappingSnapshot{
						turn:           turn,
				...
				h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, scheduleModel, openAIForwardSucceededForScheduling(result), result.FirstTokenMs)
				h.gatewayService.ReportOpenAIAccountProxyRouteOutcome(ctx, account, scheduleModel, openAIForwardSucceededForScheduling(result), false, result)
				inboundEndpoint := GetInboundEndpoint(c)
		...
		if err := h.gatewayService.ProxyResponsesWebSocketFromClient(ctx, c, wsConn, account, token, wsFirstMessage, hooks); err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if handleWSFailover(account, failoverErr, openAIWSTurnScheduleModel(account, turnChannelMapping.Load(), reqModel)) {
					continue
				}
				return
			}
			...
			if service.IsOpenAIWSCyberPolicyBlocked(err) {
				reqLog.Info("openai.websocket_cyber_policy_blocked", zap.Int64("account_id", account.ID))
				closeOpenAIClientWS(wsConn, coderws.StatusPolicyViolation, service.OpenAIWSCyberPolicyClientMessage)
				return
			}
			...
			if shouldReportOpenAIWSProxyAccountFailure(err) {
				scheduleModel := openAIWSTurnScheduleModel(account, turnChannelMapping.Load(), reqModel)
				h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, scheduleModel, false, nil)
				h.gatewayService.ReportOpenAIAccountProxyRouteOutcome(ctx, account, scheduleModel, false, true, nil)
			}
			closeStatus, closeReason := summarizeWSCloseErrorForLog(err)
...
func (h *OpenAIGatewayHandler) handleOpenAIPreviousResponseAffinityError(c *gin.Context, err error, streamStarted bool) bool {
	...
	h.handleStreamingAwareErrorWithCode(
		...
		"api_error",
		openAIPreviousResponseAffinityErrorCode,
		openAIPreviousResponseAffinityUnavailableMessage,
		streamStarted,
		false,
	)
	return true
...
func (h *OpenAIGatewayHandler) ensureForwardErrorResponse(c *gin.Context, streamStarted bool) bool {
	...
	// A compact keepalive can commit HTTP 200 without emitting a semantic SSE
	// event. The Responses stream still needs a terminal response.failed frame.
...
func closeOpenAIPreviousResponseAffinityError(conn *coderws.Conn, err error) bool {
	if !errors.Is(err, service.ErrOpenAIPreviousResponseAffinityUnavailable) {
		return false
	}
	closeOpenAIClientWS(conn, coderws.StatusTryAgainLater, openAIPreviousResponseAffinityUnavailableMessage)
	return true
}
