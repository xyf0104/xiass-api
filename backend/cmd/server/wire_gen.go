...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	refreshTokenCache, err := repository.NewRefreshTokenStore(db, redisClient, configConfig)
	if err != nil {
		return nil, err
	}
	...
	userGroupAccountAllowlistRepository := repository.NewUserGroupAccountAllowlistRepository(db)
	userGroupAccountAllowlistPolicy := service.NewUserGroupAccountAllowlistPolicy(userGroupAccountAllowlistRepository)
	accountCandidateAccessPolicy := service.ProvideAccountCandidateAccessPolicy(userGroupAccountAllowlistPolicy)
	accountCandidateAccessPolicyWiring := service.ProvideAccountCandidateAccessPolicyWiring(accountCandidateAccessPolicy, gatewayService, openAIGatewayService, geminiMessagesCompatService)
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsSystemLogSink, settingService, authCacheInvalidationWorker, apiKeyService, accountCandidateAccessPolicyWiring)
	...
	announcementEmailDeliveryRepository := repository.NewAnnouncementEmailDeliveryRepository(db)
	announcementService := service.ProvideAnnouncementService(announcementRepository, announcementReadRepository, userRepository, userSubscriptionRepository, notificationEmailService, announcementEmailDeliveryRepository)
	...
	adminServiceImpl := service.NewAdminServiceWithConfig(configConfig, userRepository, adminGroupRepository, adminAccountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService, affiliateService, compositeModelRouteRepository, compositeRouteResolver)
	adminUserHandler := admin.NewUserHandler(adminServiceImpl, concurrencyService, serviceUserPlatformQuotaRepository, billingCache, totpService, userService, settingService)
	groupCapacityService := service.NewGroupCapacityService(accountRepository, groupRepository, concurrencyService, sessionLimitCache, rpmCache)
	userGroupAccountAllowlistCandidateRepository := service.ProvideUserGroupAccountAllowlistCandidateRepository(accountRepository)
	userGroupAccountAllowlistService := service.NewUserGroupAccountAllowlistService(userGroupAccountAllowlistRepository, userGroupAccountAllowlistCandidateRepository)
	userGroupAccountRuntimeReader := service.ProvideUserGroupAccountRuntimeReader(concurrencyService)
	adminUserGroupAccountAllowlistService := service.NewAdminUserGroupAccountAllowlistService(adminServiceImpl, userGroupAccountAllowlistService, userGroupAccountAllowlistCandidateRepository, userGroupAccountRuntimeReader)
	groupHandler := handler.ProvideAdminGroupHandler(adminServiceImpl, dashboardService, groupCapacityService, adminUserGroupAccountAllowlistService)
	...
	openAIQuotaService := service.ProvideOpenAIQuotaService(accountRepository, proxyRepository, openAITokenProvider, privacyClientFactory, openAIGatewayService, settingRepository, idempotencyRepository, adminServiceImpl, rateLimitService)
	...
	accountHandler := admin.ProvideAccountHandler(configConfig, adminServiceImpl, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService)
	...
	runtimeExportService := service.NewRuntimeExportService(dbDumper, configConfig)
	...
	openAIOAuthHandler := handler.ProvideOpenAIOAuthHandler(openAIOAuthService, adminServiceImpl, openAIQuotaService, rateLimitService, redisClient, secretEncryptor, settingRepository, pixlabSMSService, compositeTokenCacheInvalidator, auditLogService)
	...
	grokOAuthHandler := admin.NewGrokOAuthHandler(grokOAuthService, adminServiceImpl, grokQuotaService, tokenRefreshService)
	proxySubscriptionPersistence := repository.NewProxySubscriptionPersistence(client)
	proxySubscriptionService := service.ProvideProxySubscriptionService(proxySubscriptionPersistence, secretEncryptor, configConfig)
	proxyHandler := handler.ProvideAdminProxyHandler(adminServiceImpl, proxySubscriptionService)
	softRouterRepository := repository.NewSoftRouterRepository(db)
	softRouterSocksGateway := service.NewSoftRouterSocksGateway()
	softRouterProxyService := service.NewSoftRouterProxyService(softRouterRepository, proxyRepository, softRouterSocksGateway)
	softRouterProxyHandler := admin.NewSoftRouterProxyHandler(softRouterProxyService)
	adminRedeemHandler := admin.NewRedeemHandler(adminServiceImpl, redeemService)
	...
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, pixlabSMSService, settingRepository, secretEncryptor)
	opsHandler := admin.NewOpsHandler(opsService)
	systemOperationLockService := service.ProvideSystemOperationLockService(idempotencyRepository, configConfig)
	systemHandler := handler.ProvideSystemHandler(updateService, dockerUpdateService, systemOperationLockService)
	adminSubscriptionHandler := admin.NewSubscriptionHandler(subscriptionService)
	usageCleanupRepository := repository.NewUsageCleanupRepository(client, db)
	usageCleanupService := service.ProvideUsageCleanupService(usageCleanupRepository, timingWheelService, dashboardAggregationService, configConfig)
	adminUsageHandler := admin.NewUsageHandler(usageService, apiKeyService, adminServiceImpl, usageCleanupService)
	...
	adminAPIKeyHandler := admin.NewAdminAPIKeyHandler(adminServiceImpl)
	...
	affiliateHandler := admin.NewAffiliateHandler(affiliateService, adminServiceImpl)
	complianceHandler := admin.NewComplianceHandler(settingService)
	auditLogHandler := admin.NewAuditLogHandler(auditLogService, totpService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	store := repository.NewPelicanBenchmarkRepository(db)
	pelicanBenchmarkHandler := admin.NewPelicanBenchmarkHandler(store, adminServiceImpl, accountTestService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, softRouterProxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService, openAIGatewayService, pelicanBenchmarkHandler)
	...
	smsReceiverHandler := handler.NewSMSReceiverHandler(pixlabSMSService)
	...
	batchImagePublicService := service.NewBatchImagePublicService(batchImageRepository, accountRepository, accountCandidateAccessPolicy, groupRepository, userGroupRateRepository, batchImageQueue, batchImageModelPricingResolver, usageBillingRepository, apiKeyAuthCacheInvalidator, configConfig, settingService)
	batchImageDownloadLimiter := repository.NewBatchImageDownloadLimiter(redisClient, configConfig)
	batchImageDownloadService := service.ProvideBatchImageDownloadService(batchImageRepository, accountRepository, batchImageDownloadLimiter, configConfig, settingService)
	batchImageCleanupService := service.ProvideBatchImageCleanupService(batchImageRepository, accountRepository, configConfig, settingService)
	batchImageHandler := handler.ProvideBatchImageHandler(batchImagePublicService, batchImageDownloadService, batchImageCleanupService, openAIGatewayHandler)
	executionNodePairingHandler := handler.NewExecutionNodePairingHandler(settingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, smsReceiverHandler, asyncImageHandler, batchImageHandler, executionNodePairingHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, db, redisClient, refreshTokenCache)
	...
	inactiveUserCleanupRepository := repository.NewInactiveUserCleanupRepository(db)
	inactiveUserCleanupService := service.ProvideInactiveUserCleanupService(adminServiceImpl, inactiveUserCleanupRepository, notificationEmailService, leaderLockCache, db, configConfig)
	batchImageWorkerRuntime := service.ProvideBatchImageWorkerRuntime(batchImageRepository, accountRepository, batchImageQueue, usageBillingRepository, usageLogRepository, batchImageModelPricingResolver, apiKeyAuthCacheInvalidator, configConfig, settingService)
