...
type Group struct {
	...
	// 成本比例（成本价/官方价），用于前端展示成本价倍数；NULL 表示不启用
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldModelPricing, group.FieldModelRouting, group.FieldModelRoutingPools, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
		...
		case group.FieldRateMultiplier, group.FieldPeakRateMultiplier, group.FieldDailyLimitUsd, group.FieldWeeklyLimitUsd, group.FieldMonthlyLimitUsd, group.FieldImageRateMultiplier, group.FieldImagePrice1k, group.FieldImagePrice2k, group.FieldImagePrice4k, group.FieldBatchImageDiscountMultiplier, group.FieldBatchImageHoldMultiplier, group.FieldVideoRateMultiplier, group.FieldVideoPrice480p, group.FieldVideoPrice720p, group.FieldVideoPrice1080p, group.FieldWebSearchPricePerCall, group.FieldCostRatio, group.FieldProfitMinMargin, group.FieldProfitSafetyBuffer:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldModelRoutingPools:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field model_routing_pools", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.ModelRoutingPools); err != nil {
					return fmt.Errorf("unmarshal field model_routing_pools: %w", err)
				}
			}
		...
		case group.FieldCostRatio:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				...
				_m.CostRatio = new(float64)
				*_m.CostRatio = value.Float64
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("model_routing_pools=")
	builder.WriteString(fmt.Sprintf("%v", _m.ModelRoutingPools))
	builder.WriteString(", ")
