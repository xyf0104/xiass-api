...
sed -n '/^github_api_curl()/,/^}/p' "$ROOT_DIR/deploy/install.sh" > "$TEMP_DIR/install-lib.sh"
...
run_api_curl() {
    CURL_ARGS_LOG="$1" HOME="$TEMP_DIR/home" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="${2:-}" \
        ...
        "$TEMP_DIR/install-lib.sh" "https://api.github.com/repos/xyf0104/xiass-api/releases/latest"
}
...
test "$(grep -Fxc 'https://api.github.com/repos/xyf0104/xiass-api/releases/latest' "$TEMP_DIR/authenticated")" -eq 1
...
test "$(grep -Fxc 'https://api.github.com/repos/xyf0104/xiass-api/releases/latest' "$TEMP_DIR/anonymous")" -eq 1
...
assert_unsafe_invocation_rejected() {
    ...
    if CURL_ARGS_LOG="$TEMP_DIR/$name" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="update-secret" \
        ...
        "$TEMP_DIR/install-lib.sh" "$@" 2>/dev/null; then
...
assert_unsafe_invocation_rejected non-api -s \
    "https://github.com/xyf0104/xiass-api/releases/download/v1/asset"
...
assert_unsafe_invocation_rejected multiple-api -s \
    "https://api.github.com/repos/xyf0104/xiass-api/releases/latest" \
    "https://api.github.com/repos/xyf0104/xiass-api/releases"
