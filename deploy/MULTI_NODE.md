...
因此，`api2` 收到一个使用主域名 Base URL 的请求时，可以从共享账号池选中 `api` 所属账号；该请求仍通过 `api` 账号绑定的 `api` 上游出口访问提供商。正常情况下不会把这个账号的上游流量改从 `api2` 发出。只有账号所属节点心跳过期、启用应急本机出口且本机出口通过检查时，才允许当前请求临时走存活节点的本机出口；账号归属、Key、计费和使用记录不改变。

## 链路

```text
客户 -> 高可用健康检查入口 -> 任一对等 XIASS 实例 -> 共享账号调度 -> 账号所属节点的固定上游出口 -> 上游
```
...
所有对等 XIASS 节点都可使用 `docker-compose.standalone.yml` 连接外部 HA 状态层。使用网页配对时，先在已有节点生成一次性邀请码，再在目标节点填写来源地址、邀请码、不同的目标节点 ID 和目标公网地址；来源节点会自动生成目标节点自己的固定出口记录并由目标宿主机 controller 写入 `.env`。手工配置场景下，每个实例的 `.env` 仍必须设置**不同的节点 ID**和该节点自己的默认出口代理 ID；共享数据库中的账号归属与账号代理在所有实例上必须可见且一致。
...
如果目标机有业务数据、来源运行时配置不完整、目标应用无法健康启动或来源节点没有收到最终确认，配对会拒绝或自动回滚，页面保持“未生产就绪”。这样可以避免两个独立 XIASS 安装分别扣费，最终造成余额和使用记录分叉。目标机加入后使用来源数据库；来源端的 PostgreSQL/Redis 本身仍应部署在独立的高可用状态层或受保护的状态主机上，应用节点断联时才有真正的容灾边界。
...
可选的 Witness 仅保护共享管理权限的临时接管，不负责 PostgreSQL/Redis 升主、数据复制或公网域名切换。未配置时保持现有行为；启用后仲裁不可达会禁止临时管理接管，但不会单独拦截客户 API、登录、只读页面或使 `/readyz` 失败。数据库与 Redis 的真实可用性检查继续保留。管理权限记录不是每笔业务事务的数据库隔离机制，不能用于宣称独立数据库之间已防止双写。

短时间断联也必须区分应用故障与整机故障：共享状态仍在线时可由健康应用继续服务；唯一状态源整机断电时仍需可用副本及可靠的数据库接管机制。已有 etcd/Patroni 仲裁应继续复用，不需再部署第二套数据库仲裁。不要用停止两台生产服务的方法验证这些边界。
