...
XIASS API supports long-lived SSE and WebSocket requests. Protect the request
...
CIDR/IP addresses that connect directly to XIASS API. An explicit empty list
...
XIASS API `server.trusted_proxies` with the Caddy address/private subnet so the
